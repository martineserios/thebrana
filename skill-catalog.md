...
| `/brana:close` | End of session | Extracts errata + process learnings, writes session state + patterns |
...
| `/brana:venture-phase [type]` | Executing a business milestone | Plans and executes: launch, hiring, fundraise, expansion, process, custom |
...
| `/brana:pipeline` | Sales tracking | Stage-aware CRM: leads, deals, conversions, follow-ups |
| `/experiment` | Growth testing | Hypothesis → test → ICE score → success criteria → results → learning |
| `/brana:financial-model` | Fundraise or monthly planning | 3-scenario revenue projection, P&L template, unit economics, cash flow |
...
| `/brana:gsheets [action]` | Direct Sheets operations | Read, write, create, list, share spreadsheets via MCP (performance-optimal) |
...
|---------|-------------|
| `/brana:apply-errata` | Applies pending doc 24 errors to affected spec docs |
| `/brana:maintain-specs` | Full correction cycle: re-evaluate reflections, apply errata layer by layer, check doc 25 |
| `/brana:re-evaluate-reflections` | Cross-checks reflection docs (08, 14) against dimension docs for gaps |
