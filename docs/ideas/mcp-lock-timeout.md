...
**Challenger gate (iteration 1) caught the fix stopping at the incident's named tools, not the class.** AC3's own wording says "other tools" (plural, systemic) — patching only 2 of the ~20 registered handlers left the majority of the tool surface, including the busiest read path (`backlog_query`) and two write paths (`memory_write`, `session_write`), sharing the identical exposure. Audited every `tools/*.rs` file directly (not just grep-matched): `backlog_query`, `backlog_get`, `backlog_stats`, `backlog_burndown`, `backlog_focus`, `backlog_stale`, `backlog_wave_get`, `backlog_wave_list`, `memory_index`, `memory_write`, `session_read`, and `session_write` all ran synchronous I/O (file reads, a directory-tree scan in `memory_index`, a `git branch` subprocess call in `session_write`, or file writes in `memory_write`/`session_write`) directly inline on pmcp's single dispatch task. All 12 now wrapped in `spawn_blocking`, matching the same pattern. `agy_delegate.rs` was checked and found already correct — it uses `tokio::process::Command` (genuinely async) plus an explicit `tokio::time::timeout(120s)`, not a blocking call. Full suite (813 tests) and `validate.sh` re-verified green after the extended sweep.
