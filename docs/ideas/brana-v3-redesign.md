...
2. **Contract-driven loops.** The task is the machine-readable interface: `execution:` marker (default-deny eligibility), `AC:` lines (mechanical verification), `blocked_by`/priority/effort (sequencing + shape features). A loop is only as verifiable as its contract is authored — so the cockpit helps write ACs at task creation, and AC adoption is tracked work (~11% coverage today).
