...
| **2 · LEARN AUTONOMOUS** | The async worker on the compute chain (agy-first → Claude engine-switch, bounded by a hard per-run token ceiling with checkpoint/resume — never unboundedly deferred). Curation gate + run/token ceiling *inside this wave*. **Tier B (scoped-mutation) observe-invariant test for the worker** — proves LEARN's memory writes are inert (gate nothing until human promotion), not a Tier-A zero-mutation test (principle 6). Honest label: proves unattended execution on a low-risk loop. Resolve close-FULL scope per ADR-052 (escape hatch survives). | agy skip-and-defer stall path · close FULL sync-extraction weight |
...
| **5 · GRADUATE + CUT** | Ships the graduation **machinery** — outcome ledger, transparent per-shape rule table, auto-demotion — not a graduation guarantee on a wave-5 timeline (resolved 2026-07-21 challenge finding #1, t-2285: at solo throughput ≥1 task/day, 2 weeks yields only 14 outcomes across ALL shapes, nowhere near ≥50 per-shape). Requires shape coarsening as the only path any shape becomes reachable at all — provisional target **≤10 shapes**, a floor-based heuristic, not a derived optimum (computed from the stated *floor* throughput of ≥1/day; actual post-wave-4 cockpit throughput may run higher and loosen this, and principle 5's per-shape K is not yet numerically fixed — re-derive both once wave-4 ships real data). Even then, a shape's first graduation is realistically **months** past wave-4 shipping, not a wave-5 checkbox. Core vs process-packs boundary (→ t-2090). Hook tier-model page. | v2 remnants retired against the v3 spec |
