...
| **Teams** | Peer sessions with shared task list + SendMessage | Agents can *argue*, claim work, coordinate | Experimental; highest token cost | Competing hypotheses, cross-layer changes, persistent challenger | **Unused** — no agent defined here has `SendMessage` in its tool list yet (confirmed for `challenger`, t-3150); spawning one as a Team peer that must reply directly is structurally stuck regardless of how long you wait. Use the standard one-shot `Agent()` completion-path spawn (`system/skills/_shared/challenger-gate.md`) instead until an agent is deliberately built for Teams |
...
| 7 | Teams experiment: persistent challenger | L3 | Unused — `challenger.md` has no `SendMessage` tool, so it cannot reply as a Team peer today (t-3150) |
