...
1. **Autonomous tier: ruflo autopilot vs native `claude -p` background?** Partially resolved by a controlled probe (2026-06-19): ruflo autopilot's persistence loop is **real** — `enable`→`check` returns `CONTINUE: 95/234 tasks remaining (iteration 1/50)` (real counts), `predict` picks a real next task from the queue, `disable` restores `ALLOW STOP`. It works as a **Stop-hook re-engagement loop** (blocks a host CC session from ending, feeds the next queued task, up to max-iterations/timeout). Caveats: task *selection* is queue/heuristic, not ML (`predict` confidence is a hardcoded `0.5 — learning not available`); it requires a host CC session with the stop-hook wired plus tasks in its sources (team-tasks/swarm-tasks/file-checklist). **Lean recommendation:** for brana, a native loop over `tasks.json` (via `/loop` or cron + `claude -p`) is likely simpler and more controllable than wiring autopilot's stop-hook + syncing its task sources — but autopilot is a working fallback if we want "until done" persistence without building it. Decide when the autonomous tier is actually needed.
   **RESOLVED (2026-06-20, t-2141 substrate audit):** native `/loop + claude -p` over the backlog wins. Live evidence: a 4-iteration probe picked the first-pending task each time, did the work, updated state, and self-halted on ALLDONE for $0.18 — backlog-native, zero wiring. autopilot's `enable` installs no Stop hook (you must wire one), its sources are flat JSON files separate from the backlog, and its task-pick is the same heuristic the loop does for free. See [substrate-leverage-audit](../../research/substrate-leverage-audit.md). Implementation tracked in t-2140.
...
3. **CLI `--claude` spawn and v3.37 Meta LLM Proxy — worth reopening now that conditions have changed?** (raised by t-2763, re-analysis of the two subscription-real ruflo paths set aside above). **RESOLVED — CLOSED, not just deferred (2026-08-12/13, t-2763 + live re-confirmation same day).** Both stay closed:
...
- **Use ruflo `--claude` for autonomous/background work (the niche this ADR originally granted it).** Reconsidered and rejected 2026-08-12/13 (t-2763, Open Question 3 above) — no unique capability over native `claude -p`, and no worktree isolation makes it structurally unsafe for unsupervised repo-touching work.
