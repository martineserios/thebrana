...
- 2026-06-12: §2-5 shipped (t-1998, e7e64f51): channel registry load + routing resolve, adapters, two-phase dispatch (select/send/commit), `brana remind due --dispatch`, `brana notify send/channels`, scheduler firebreak comment. Pending: t-1999 (scheduler timer), t-2000 (docs).
- 2026-07-22: §6 scheduler wiring shipped (t-1999) — reminder-dispatch job deployed every 30 minutes, notify-channels registry created, first-run backfill handled, verified end-to-end in production.
- 2026-08-24: [ADR-089](ADR-089-integral-temporal-model.md) consolidates this ADR's dispatch semantics with ADR-051's store decisions into a cross-cutting temporal model shared with tasks and scheduler jobs (unified eligibility predicate, compute-don't-copy, snooze≠deadline firewall, per-priority lead times, staged rollout) — this ADR is unchanged, ADR-089 names the shared machinery on top of it and stages the not-yet-built recurrence/task-linkage work (t-2074)
