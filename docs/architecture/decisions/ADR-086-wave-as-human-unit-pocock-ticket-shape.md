...
[the-brana.md](../the-brana.md) (§Scale — human as low-pass filter; §Cycle → Mechanics — four primitives; absorbed from the retired wave-pipeline.md, t-3028) ·
...
**Amendment 2026-08-31 (t-3252): ship-certificate membership is deliberately bespoke-inclusive, not a bug.** Two membership definitions coexist by design: `wave_counts`/`wave_pull_decision` apply `matches_as_pending AND NOT bespoke-owned` — the *pull-side* precedence rule this section states, which exists so a bespoke wave's own pump gets first claim on an overlapping task. `wave_ship`'s `exec_check`/`build_ship_report` use bare `matches_as_pending` — full selector membership, bespoke-owned tasks included. This is not the same rule applied inconsistently; it is two different questions. Pull asks "whose pump claims this task next" (precedence matters — a wave's WIP would double-count a task another wave already owns). Ship asks "did everything this selector names get done" (precedence is irrelevant — excluding a bespoke-owned task from the certificate would let a standing-wave ship report PASS while that task is still in flight elsewhere, recreating the vacuous-PASS class t-3249 closed). Ship certificates therefore always evaluate against full selector membership, independent of which wave currently owns the pull. No code change: `wave.rs:537`/`:274` (pull, exclusive) and `wave_ship.rs:268`/`:369` (ship, inclusive) are both correct for their respective questions.
...
## Candidate tasks (each S/M, one wave slot) — filed 2026-08-24 under epic t-2337, tag `the-brana-closeout`; verifier follow-up **t-3166** (classify() cancelled-blocker semantics)

| # | Task | Effort | Amends |
|---|---|---|---|
| T1 (**t-3160**) | ✅ **shipped 2026-08-30** (unblocked same day t-2834's EXPAND verdict landed) — `derive_role()` in `brana-core/src/tasks/role.rs`; `WaveSelector::Role` joins `tag:`/`parent:` at the single parse point (ADR-080 §1); `wave_pull_decision` calls the shared derivation, no second inline `ac_state`/`parked` check — a genuine behavior fix, not just refactor: the old check never looked at `tag:human` at all, so a human-tagged approved task could previously be pulled autonomously; `--role` wired into `brana backlog query` + `get` output; `set role` rejected via `set_field`'s existing allowlist (same mechanism as `epic`, no new code needed). Doc: [backlog-v3-schema.md §Role view](../features/backlog-v3-schema.md#role-view--derived-triage-roles-adr-086-3-t-3160). | M | brana-core query, wave selector parser (ADR-080 §1 single parse point — one `Role` arm, all consumers route through it) |
| T2 (**t-3043**) | — (filed from ADR-079's amendment; not duplicated here — F8). Close t-3043 against §4's wording | S | ADR-079 §2 |
| T3 (**t-3161**) | ✅ **shipped 2026-08-30** — `wave-standing` created draining (selector `role:ready-for-agent`, no contract/gate, provisional `wip_limit` 2); `wave_pull_decision` (wave.rs) gained the waves array: role-selector pulls sort by priority (P0 first, absent last) then `created` ascending (F4), defer candidates matched by any other *draining* tag:/parent: wave (bespoke precedence — `NoneEligible` grew a `deferred` count; queued bespoke waves don't shadow), and count wip live as in_progress tasks that would derive the role were they pending minus bespoke-owned ones (an in_progress task derives `claimed`, so the naive selector match counted live=0 forever and the provisional `wip_limit` 2 could never bind — fixed, not just ordered). Bespoke pulls unchanged (array order, deferred always 0). Docs: drain-loop.md §Standing wave, epic-drain.md counts line, ADR-080 §3 addendum. | S | drain-loop.md, ADR-080 §3 |
| T4 (**t-3162**) | ✅ **shipped 2026-08-29** — `CHECK:` lines parsed against vocabulary v1 (probe A1 `CHECK-EXEMPT:` + A2 prose band included; parser `brana-core/src/wave_ship.rs`, moved from brana-cli by its t-3234 fast-follow — see below); `wave ship` evaluates-and-shows then flips (operator decision 2026-08-29: display never gates — red check informs, human's invocation ships); `contract` runner-denied (hook + tests); D2/D5 amended. **Fast-follow t-3234 (shipped 2026-08-30):** the MCP `backlog_wave_set` tool used to flip `status: shipped` with zero evaluation — a second, divergent ship surface. Fixed by moving the evaluator into `brana-core::wave_ship::build_ship_report` so both `cmd_wave_ship` (CLI) and `backlog_wave_set` (MCP, on `field:status value:shipped`) call the same function; MCP's result now carries a `check_report` array identically to the CLI's printed lines. | M | backlog-v3-schema D2/D5, runner-verb-guard.sh |
| T5 (**t-3163**) | ✅ **shipped 2026-08-29** — `docs/agents/issue-tracker.md` for thebrana + `/brana:align` generator; `CONTEXT.md` generator in `/brana:reconcile` — **both generators are new** (`/brana:reconcile` has no generator phase today; F11) | M | ADR-084 §3 (supplies its tracker half) |
| T6 (**t-3164**) | ✅ **shipped 2026-08-29** (`docs/research/2026-08-29-field-usage-audit.md`; follow-up t-3233 filed: query undercounts backlog) — structured-field usage audit (`brana backlog query --output json`, fill rate per field, `context`/`notes`/`description`/AC excluded); report only, diet decided after | S | backlog-v3-schema §Relationship |
| T7 (**t-3165**) | ✅ **shipped 2026-08-29** — sizing rules + pointer discipline written into `drain-loop.md`, `tactical-context.md`, `task-convention.md`; phase/milestone-creation warning in the CLI | S | task-convention.md, tactical-context.md |
...
- 2026-08-31: resolved t-3252 (panel SPLIT verdict) — documented in §5 that ship-certificate membership (bespoke-inclusive) and `wave_counts` pull membership (bespoke-exclusive) answer different questions by design, not a divergent-code defect; no code change.
