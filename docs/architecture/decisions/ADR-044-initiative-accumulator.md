...
**`epic: Option<String>`** added to `SessionState` — populated by close.md Step 9c, read by sitrep.md Source 4b.

**CLI:** `brana session initiative <upsert|read|archive>` — thin wrapper around brana-core functions.

**Epic detection at close (Step 9c, 3-tier cascade):**
1. Tier 2a: `brana backlog query --status in_progress` → collect `epic` fields.
2. Tier 2c: branch name parsing as fallback.
3. Tier 3: `AskUserQuestion` when 0 or 2+ epics detected.
...
- **`brana backlog complete --epic X`** — separate task.
...
- No breaking change to existing session-state.json consumers — `epic` field is optional.
