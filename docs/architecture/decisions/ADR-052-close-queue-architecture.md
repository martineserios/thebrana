...
**Source:** async-close design (docs/ideas/drained/async-close-design.md Q1–Q3) + plan challenger review 2026-06-10 (verdict: PROCEED WITH CHANGES — 4 CRITICAL, 4 HIGH findings, all resolved here)
...
- Output missing, empty, or unparseable as the schema above → `brana close-queue mark-failed <id> --error …`; **never** write partial results, **never** mark processed
  - **Exception — quota exhaustion (t-2085, 2026-06-14):** when agy exits 0 with empty stdout (agy 1.0.8 regression: `RESOURCE_EXHAUSTED 429` silently produces no output), or when agy exits non-zero and stderr/output contains `429`/`rate-limit`/`RESOURCE_EXHAUSTED`, the entry is **skipped** (not failed) and the loop breaks. Rationale: quota exhaustion is a transient infrastructure limit, not a data error — burning retry budget on it prevents legitimate failure recovery. Entries remain unprocessed and are retried at the next nightly run. Error category: `quota-exhausted:` (not `agy-empty-output:` or `schema-invalid:`). See also [ADR-041](ADR-041-agy-invocation-contract.md) for the agy quota-silent-exit field note.
