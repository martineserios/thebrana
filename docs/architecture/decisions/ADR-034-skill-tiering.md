...
**Amended (2026-06-10, t-1942):** The transitional exception is closed — the big four are phase-split. Each is a slim SKILL.md (flow overview, rules, machine-readable `<!-- PHASES -->` registry mapping steps/subcommands/scopes to files) plus per-phase bodies in `system/skills/{name}/phases/*.md`, every phase file ≤400 lines (SKILL.md ≤500). The SKILL.md Phase Protocol carries three load-bearing rules: Read the phase file at every step boundary (never execute a phase from memory); Read chained subcommands'/scopes' files at their boundaries; on resume-after-compression, Read the current step's phase file before continuing. Shared sections referenced by multiple strategies live in `system/skills/_shared/` (challenger-gate.md was extracted before the split so bug-fix/refactor strategies don't lose gate enforcement across files). No stubs exist anywhere; `system/procedures/` holds only migrate.md. Enforcement: `tests/skills/test_skill_inline_layout.sh` (empty allowlist) + `tests/skills/test_skill_phase_layout.sh` (registry, size caps, resume protocol, no orphans). Tooling reads the layout-agnostic **effective body** (SKILL.md + phases in registry order): `tests/lib/effective_body.sh` and validate.sh `effective_body()`. Per-phase files carry their own ToolSearch/ruflo preambles (Checks 36/40/45 scan `skills/*/phases/*.md`).
...
**Rollback:** revert the t-1941 merge commit, then re-run the bootstrap sync. Both steps are required — reverting without re-syncing leaves the cache on the inlined layout. Same recipe for t-1942 (revert merge + re-sync); the effective-body resolvers keep tests/validate green on either side of the revert.
