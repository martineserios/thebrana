...
## Context
The `/brana:memory review --audit` integration test (t-039) found 10 contradictions in docs — 7 of which were stale counts (skills, dimensions, hooks). These drift silently when components are added/removed but docs aren't updated.
...
3. Add delegation-routing trigger: after `/brana:maintain-specs` cascades → suggest `/brana:memory review --audit` on touched docs.
