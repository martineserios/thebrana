...
**Exception — Claude Code auto-memory system.** The system-prompt auto-memory instructions
direct the model to write typed memory files directly via the `Write`/`Edit` tools to
`~/.claude/projects/*/memory/`. This is an intentional bypass: the auto-memory layer
predates this gateway and runs outside skill procedures. The `memory-write-gate.sh`
PreToolUse hook explicitly passes through any write to `$HOME/.claude/projects/*/memory/**`
(see commit `8f1382e`). Do not route these writes through `brana memory write`.
