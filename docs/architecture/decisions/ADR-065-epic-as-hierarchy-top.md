...
- **`sync.rs`'s `keep_streams` allowlist was a second, undocumented live consumer** (t-2325, found by the Challenger gate 2026-07-23): `plan_sync()` filtered every task on `task["stream"]` matching a non-empty `keep_streams` list (live default: `roadmap`/`tech-debt`/`bugs`). Dropping `stream` without also removing this filter would have made the comparison against `""` permanently fail, silently zeroing out `brana sync` (no issues created or closed) rather than erroring. t-2325 removed the filter, the `SyncConfig.keep_streams`/`label_stream` fields, and the cosmetic `stream` reads in `build_issue_body`/`build_labels`, and added `plan_sync()`'s first unit tests. This item was missed in the original Consequences list above — captured here for future field-retirements to check config-driven *filters*, not just labels/displays, when tracing a field's read consumers.
- **Navigation simplifies**: default depth `epic → task`; one active epic. (The WIP cap named here was retired 2026-08-12 — see Amendment below.)
...
## Amendment (2026-08-12, t-2727): WIP cap retired

Decision item 2 above gave the epic node a `wip_limit` (D4 in the [feature spec](../features/backlog-v3-schema.md#decisions-resolved-2026-07-20), advisory during pilot). That cap is **retired**, not retuned or enforced — see the feature spec's D4 entry for full rationale and evidence. Summary: epics turned out to be unbounded groupings in actual usage (9/55 live epics 4-7x over the default cap), matching this ADR's own Context — "epic = 'what we're building'" was never meant to imply small. WIP/concurrency control, if wanted, moves to a different unit — waves (D3) — as a follow-up (t-2782), not designed here. `wip_limit` is removed from the schema (added to `RETIRED_FIELDS`); `check_epic_wip_cap()` is deleted.
