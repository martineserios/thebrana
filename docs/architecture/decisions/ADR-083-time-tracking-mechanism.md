...
- **Combined CLOSE-latency budget with `build-receipts.md`'s `mint`.** `mint` already runs at CLOSE step 1 and is measured against its own kill-threshold K2 (`build-receipts.md`: median wall-clock `mint` adds to CLOSE > 60s → make `mint` opt-in). This ADR adds a second CLOSE-side write (bracket-close + aggregation) at the same integration point. K2's accounting is `mint`-only — it will not see latency this design adds. Track the two together as one combined CLOSE-latency budget, not two independently-passing measurements that can jointly regress CLOSE wall-clock unnoticed; if the combined figure trends toward K2's 60s line, that is a signal even if `mint` alone stays under it.
- **Prompt-cache sharing across forks — token-cost double-counting risk, not blocking.** Metric 1 already excludes subagent/fork time from the *duration* sum (v1 scope, above). The parallel risk on the *token-cost* side of Metric 1 (not yet built — cost aggregation shares this same bracket mechanism per the idea doc) is that prompt-cache reads shared between a parent session and its forked subagents could be attributed to both, double-counting cache-read tokens across the two. Since forks are already excluded from v1's duration sum for a different reason (#3 above), this does not block v1, but the cost-aggregation implementation (Next Steps #4/#6) must not assume token counts are cleanly additive across parent/fork boundaries without checking this.
