...
**H2 — git-env scrubbing.** Any subprocess the marker-writing step invokes that itself runs git must not inherit `GIT_DIR`/`GIT_WORK_TREE`/`GIT_INDEX_FILE`/`GIT_OBJECT_DIRECTORY`/`GIT_COMMON_DIR` — a leaked value can redirect writes into a foreign repository (`pattern_git-hook-env-leaks-into-executed-tests`; live incident precedent in `build-receipts.md` H2). `GIT_COMMON_DIR` matters twice here too: it is what the storage path itself resolves through.
