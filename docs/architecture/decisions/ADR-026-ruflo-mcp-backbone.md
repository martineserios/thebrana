...
**CORRECTION (2026-08-12, t-2757, live-probed on ruflo v3.34.0):** the "in-memory Map stub" cause above is superseded — the entire `agentdb_*` family (not just hierarchical-store/recall) is unavailable on our install for a different, more fundamental reason: our installed `@claude-flow/memory` package (v3.0.0-alpha.21) double-exports `ControllerRegistry` (a legacy `controller-registry-shim.js` re-export alongside the real one), which throws a Node ESM "Duplicate export" error on `import('@claude-flow/memory')`. `memory-bridge.js:164` catches this and permanently sets `bridgeAvailable = false`, so `agentdb_health`, `agentdb_controllers`, `agentdb_hierarchical-store/recall`, and native `agentdb_pattern-store` all report "AgentDB bridge not available" — live-confirmed via `agentdb_health` returning `{available: false}` and a direct `agentdb_hierarchical-store` call failing outright. The **same bug** causes `memory_search(smart:true)` to silently fall back to `smart:false` (flags `smartFallback: "...Duplicate export of 'ControllerRegistry'"` — see t-2756). Upstream HEAD (v3.38-era source) has dropped the shim re-export — single export only — so this is genuinely fixed there, but not in v3.34. **Net: the classification stands (BROKEN), but the fix path changed** — this is one packaging bug fixable by an upstream upgrade, not four separate unfixable stubs. Not a reason to accelerate the upgrade alone (see t-2763 — the corruption root cause #2512 remains the actual upgrade gate). Distillation's own causal-edge/episode writes (451+ entries, running every 30min) are unaffected — they go through a separate direct-`better-sqlite3` path, not this bridge.
