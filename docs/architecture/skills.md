...
| ship | execution | model-invoked | Was user-invoked-only (audit t-2832: a deploy is the canonical irreversible action); flipped by operator decision 2026-09-20 (t-3348) so a plain "ship it" is not a dead end — Claude can start it. The flag only ever blocked the `Skill` tool (a friction nudge, not a gate). The safety net is unchanged and is ship's own: mandatory `AskUserQuestion` gates (pre-flight, Gate 3, merge) and the rule "never auto-deploy without user confirmation". Cost: ship's description is now loaded every turn. |
