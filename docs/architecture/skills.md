...
| provisioning-wizard | core | model-invoked | Thin adapter (t-2836, ADR-084 vendor+wrap pilot expansion) wrapping the vendored `wizard` organ; generates ephemeral (`/tmp` by default) interactive bash scripts for human-only provisioning steps. Not ambiently triggered — reached only via an explicit `Skill(skill: "provisioning-wizard")` call when a build hits a wall only a human can pass. Docs corrected 2026-09-20 (t-3348): this row said user-invoked-only, but the skill never set the flag and its own rationale has the model reach it through an explicit `Skill` call. |
| sitrep | core | model-invoked | Read-only situational-awareness/status recovery. |
| **two-axis-review** | core | **user-invoked-only** | Thin adapter (t-2835, ADR-084 vendor+wrap pilot expansion) wrapping the vendored `code-review` organ; runs Standards and Spec as two parallel, never-merged review axes. `disable-model-invocation: true` in frontmatter — reached only via an explicit `Skill(skill: "two-axis-review")` call, not autonomous firing, and deliberately out of scope of `challenger`/`challenger-calibration.md` (pre-commitment plan/architecture review, not post-code diff review). |
...
| ship | execution | model-invoked | Was user-invoked-only (audit t-2832: a deploy is the canonical irreversible action); flipped by operator decision 2026-09-20 (t-3348) so a plain "ship it" is not a dead end — Claude can start it. The flag only ever blocked the `Skill` tool (a friction nudge, not a gate). The safety net is ship's own: the pre-flight "Deploy?" prompt, Gate 3 (asks on HIGH findings) and a **merge gate** that must be answered "Merge now" before `gh pr merge`/bootstrap/publish. Every gate fails closed when `AskUserQuestion` is unavailable, Abort is listed first, the merge sits in its own fenced block pinned with `--match-head-commit`, and any stop clears the goal — plus the rule "never auto-deploy without user confirmation". (t-3348 first claimed the merge gate existed; it did not until t-3366, and `tests/scripts/test-skill-invocation-mode.sh` now asserts it.) Cost: ship's description is now loaded every turn. |
