...
| **provisioning-wizard** | core | **user-invoked-only** | Thin adapter (t-2836, ADR-084 vendor+wrap pilot expansion) wrapping the vendored `wizard` organ; generates ephemeral (`/tmp` by default) interactive bash scripts for human-only provisioning steps. Not ambiently triggered — reached only via an explicit `Skill(skill: "provisioning-wizard")` call when a build hits a wall only a human can pass. |
