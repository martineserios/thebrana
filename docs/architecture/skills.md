...
| ship | execution | model-invoked | Was user-invoked-only (audit t-2832: a deploy is the canonical irreversible action); flipped by operator decision 2026-09-20 (t-3348) so a plain "ship it" is not a dead end — Claude can start it. The flag only ever blocked the `Skill` tool (a friction nudge, not a gate). The safety net is ship's own: the pre-flight "Deploy?" prompt, Gate 3 (asks on HIGH findings) and a **merge gate** that must be answered "Merge now" before `gh pr merge`/bootstrap/publish and fails closed when `AskUserQuestion` is unavailable — plus the rule "never auto-deploy without user confirmation". (t-3348 first claimed the merge gate existed; it did not until t-3366, and `tests/scripts/test-skill-invocation-mode.sh` now asserts it.) Cost: ship's description is now loaded every turn. |
