...
- **Wayfinder ≈ drain loop (cousins, not the same thing).** Pocock's wayfinder and brana's wave/drain loop are both claim-before-work frontiers over blocking-edge graphs, but they optimize different outputs: wayfinder produces decision tickets, the drain loop produces shipped code. Wayfinder is human-cranked — no pump, no gate — while the drain loop is a pump behind human valves. Don't conflate them when porting ideas either way. `[research 2026-08-13-matt-pocock-skill-system.md §2 · t-2830 · t-2849]`
...
| **Seven-step skeleton** | A ring's loop shape: ORIENT→SELECT→ACT→MEASURE→JUDGE→ASSIMILATE→RESTART; plain gloss = context→action→feedback. Model/effort routing (which tier a step runs at) only ever applies to ORIENT/ACT/JUDGE/ASSIMILATE — SELECT/MEASURE/RESTART are deterministic control-flow at every ring by construction (the queue and gauge laws), never a model call. | [60-agent-loop-architecture.md](../../../brana-knowledge/dimensions/60-agent-loop-architecture.md), [research/2026-08-30-model-effort-routing.md](../research/2026-08-30-model-effort-routing.md) |
