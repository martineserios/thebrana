...
### Check 27: MCP Wrapper Script exec Pattern

MCP wrapper scripts in `system/scripts/` that invoke MCP servers must use `exec`, not background-and-wait (`binary & wait`). The background pattern breaks JSON-RPC stdin delivery — the pipe is not kept alive for MCP stdio.

- **PASS:** all MCP wrapper scripts use `exec`
- **FAIL:** any wrapper script matches the background+wait anti-pattern (`ruflo|node|npx|python|uvx` followed by `&` at end of line)

**Common failures:** copying a generic server-launcher template that uses `&` + `wait` instead of `exec`. Fix: replace `./server &` with `exec ./server`.

### Check 28: No Bare python3 in system/hooks/

Hook scripts must use `jq` or `uv run python3` rather than bare `python3`. Bare `python3` is fragile in hook subprocesses where PATH is restricted and virtual environments are not active.

- **PASS:** no `python3` invocations (excluding `uv run python3`) in `system/hooks/*.sh`
- **FAIL:** any `.sh` file in `system/hooks/` (excluding `tests/`) contains bare `python3`

**Exclusion:** `--exclude-dir=tests` is required. Test scripts in `system/hooks/tests/` legitimately use `python3` to build fixture inputs. Without the exclusion, Check 28 produces false positives on every test runner invocation.

**Common failures:** using `python3 -c` for quick JSON parsing instead of `jq`.

### Check 29: Reference Docs Up to Date

Verifies that `docs/reference/*.md` (auto-generated by `brana reference generate`) match the current `system/` state. Catches silent drift when hooks.json schema or frontmatter sources change without a regeneration step.

- **PASS:** `brana reference generate --check` exits 0 (no drift detected)
- **FAIL:** reference docs are out of date — run `brana reference generate` to fix
- **Skipped:** if the `brana` CLI is not in PATH (warns but does not fail)

**When to run:** after any edit to `docs/architecture/hooks.md`, `system/skills/*/SKILL.md`, or `system/agents/*.md`.

### Check 30: brana CLI Calls Wrapped in cd GIT_ROOT

All hook scripts `cd /tmp` at startup. `brana` subcommands resolve the project from CWD, so bare calls from `/tmp` return empty results. Every `$BRANA` or bare `brana` call in a hook must be inside a `(cd "${GIT_ROOT:-...}" && ...)` subshell.

- **PASS:** all `$BRANA …` and bare `brana …` lines in `system/hooks/*.sh` are preceded by a `cd` on the same line or are inside a subshell with `cd`
- **FAIL:** any unwrapped `$BRANA` or `brana` call detected

**Common failures:** adding a new `brana backlog query` call in a hook without wrapping it. Fix: `(cd "${GIT_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || echo /tmp)}" && $BRANA backlog query ...)`.
