...
### 2026-06-09: ruflo MCP reconnect failure from stale lock+PID files — manual recovery required (→ t-1921)
When a ruflo MCP server process dies without cleanup (crash, SIGKILL, OOM), it leaves stale `~/.swarm/ruflo-mcp.lock` and `~/.swarm/ruflo-mcp.pid` files. The next session startup sees the lock file, assumes the process is running, and skips restart. All `mcp__ruflo__*` calls return `-32000` connection errors. Recovery: `kill $(cat ~/.swarm/ruflo-mcp.pid); rm -f ~/.swarm/ruflo-mcp.lock ~/.swarm/ruflo-mcp.pid` then `/mcp` reconnect. **Rule**: if any ruflo MCP tool returns -32000 at session start, the first diagnostic step is checking stale lock files, not network or config issues. The same pattern (stale binary detection via `kill -0 $PID`) exists in `session-start.sh` for other binaries — t-1921 tracks adding this check for the ruflo lock file specifically.
Source: t-1922 session 2026-06-09 [→ t-1921]
