...
### 2026-06-04: rust-skills-guard.sh must be loaded before the first .rs edit each session
`rust-skills-guard.sh` blocks all Write/Edit calls to `*.rs` files until `/brana:rust-skills` has been loaded this session (sentinel: `/tmp/brana-rust-skills-loaded-{SESSION_ID}`). On a fresh session the sentinel is absent — every `.rs` edit attempt is blocked and the agent loop stops with "Execution stopped by hook", requiring the user to type "continue" to resume. Fix: always run `/brana:rust-skills` as the first step when the session involves Rust edits (epic branch references a Rust crate, or task spec mentions `.rs` files). Bypass sentinel `/tmp/brana-rust-skills-guard-bypass` works but is not the intended path.
