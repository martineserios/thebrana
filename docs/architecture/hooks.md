...
| `post-hooks-json.sh` | PostToolUse | `Write\|Edit` | Reference doc regeneration: triggers `brana reference generate` when `hooks.json`, `SKILL.md`, or agent `.md` files are written. Keeps `docs/reference/` in sync with source metadata. |
| `memory-index-sync.sh` | PostToolUse | `Write\|Edit` | Sync `MEMORY.md` index after a typed memory file is written. Appends a one-line pointer if not already present — dedup-only, preserves existing sections. |
...
| `config-drift.sh` | _(sub-script)_ | called by `session-start.sh` | Detect drift between `system/` source files and deployed `~/.claude/` files. Also scans `~/.claude.json` for ADR-033 violations. Surfaced via `DRIFT_CONTEXT` at session start. |
| `session-end-metrics.sh` | _(sub-script)_ | invoked by `session-end.sh` | Compute flywheel metrics (tool call counts, correction rate, context usage) from session JSONL and store to ruflo |
| `session-end-persist.sh` | _(sub-script)_ | invoked by `session-end.sh` | Store session summary to ruflo (L1 memory) and auto-memory (L0) |
| `session-end-drift.sh` | _(sub-script)_ | invoked by `session-end.sh` | Post-session system sync: refresh spec-graph, update decisions log, detect behavioral drift |
| `session-end-pattern-promotion.sh` | _(sub-script)_ | invoked by `session-end.sh` | Promote or demote pattern confidence scores based on session correction rate (t-203) |
...
| `pre-compact.sh` | PreCompact | `""` (all) | Inject active task context, branch, and recent decisions into the post-compaction window so Claude resumes with orientation |
| `config-change-guard.sh` | ConfigChange | `""` (all) | Block `ANTHROPIC_BASE_URL` manipulation (CVE-2026-21852: prompt/key exfiltration via attacker-controlled endpoint). Audits all in-session config changes. |
