...
| `memory-write-gate.sh` | PreToolUse | `Write\|Edit` | Block typed-memory writes (`{type}_{slug}*.md`) to `~/.claude/projects/*/memory/` unless sentinel `/tmp/brana-memory-write-active` is present. **Pass-through exception**: writes to `~/.claude/projects/*/memory/` from the CC auto-memory system (no sentinel) are allowed unconditionally — the gate only intercepts brana-procedure-driven writes. Spec: ADR-038. |
