...
| `memory-write-gate.sh` | PreToolUse | `Write\|Edit` | Block brana-procedure-driven typed-memory writes (`{type}_{slug}*.md`) in any `*/memory/` directory unless sentinel `/tmp/brana-memory-write-active` is present. **Pass-through exception**: all paths under `~/.claude/` or `/home/*/.claude/` (CC auto-memory) pass unconditionally. Returns `permissionDecision:deny` (not `continue:false`) so `continueOnBlock:true` in hooks.json is respected — see field note 2026-06-04. Spec: ADR-038. |
