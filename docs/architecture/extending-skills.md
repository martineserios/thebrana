...
| `disable-model-invocation` | No | Set to `true` if the skill's effects are destructive, one-shot, external, or hard to reverse (deploy, archive/delete data, publish, modify shared registries — one deliberate exception: `ship` is model-invocable behind its mandatory, fail-closed merge gate, see the audit table in [skills.md](skills.md), t-3348/t-3366) — the agent can then only reach it via the user typing `/brana:{name}`, not autonomously. Omit for skills that are safe, cheap, and reversible enough for the agent to fire opportunistically. See the audited classification in [Skills Architecture](skills.md#invocation-mode-audit-t-2832-2026-08-17). |

> **Hazard (t-2954): don't stack `context: fork` with an in-skill Agent-tool fan-out that awaits its children.** The harness has no documented TTL or reaping contract for forked skill contexts — none exists anywhere in this repo. If a `context: fork` skill itself spawns further Agent-tool sub-agents and tries to synchronously "await all N" across turns (the pattern in `system/skills/_shared/adversarial-hive-mind.md`), the forked orchestrator can vanish mid-wait, silently discarding its children's results (confirmed twice in `/brana:challenge`, t-2954 — root-caused and `context: fork` removed there). Awaiting Agent-tool notifications across turns is only a documented-safe pattern when the awaiting context is a live interactive session, not a fork of one. If a skill both needs backgroundability *and* needs to fan out to sub-agents internally, prefer the `Workflow` tool's `agent()`/`parallel()` primitives (which block within the script's own execution) over raw `context: fork` + notification-waited `Agent` calls.
