...
- **t-2764 (done, 2026-08-12) — `backlog_query(epic:...)` resolution mechanism, traced end to end, source-cited.** No new mechanism; this closes the gap between "observed behavior" and "verified against the code that produces it," per a live confusion where `backlog_set(field: "epic")` was tried and rejected with `unknown field: epic`. There is no queryable `epic` field on a task, stored or otherwise — `epic` is a query-time **parameter**, never a task **field**. Full chain, one function, three callers:
  ...
  - **Distinct from `active_epic`.** `assert_active_epic_resolves()` (`query.rs:110-124`, backs `backlog focus`/`backlog next`) is a *separate* function that checks whether the single `tasks-config.json` `active_epic` pointer names a real epic node — precondition validation on one pointer, not the per-task membership resolver above. The two are easy to conflate (both compile a slug against epic nodes) but answer different questions and share no code.
