...
- **Migration deferred, not run.** `collapse-level-epic-v3.py` is written, tested against a full copy of the live dataset, and committed — but `--write` has deliberately not been run against the actual live `tasks.json`. Every write-path/read-path change (write-path sealing, `active_epic` resolution, epic-status validation, epic-node defaults) was built to tolerate BOTH the pre-migration shape (flat `epic` string) and the post-migration shape (`type:"epic"` node) so nothing regresses while the migration sits unrun. Running it for real is a deliberate follow-up, not part of this epic's close.
