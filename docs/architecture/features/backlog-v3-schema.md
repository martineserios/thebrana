...
- **WIP cap — retired 2026-08-12 (D4 below).** Epics are unbounded groupings ("what we're building"), not concurrency-limited; 9/55 live epics in this project alone sat 4-7x over the old default-10 cap with no false-positive-free way to satisfy it. WIP control, if wanted, belongs on a smaller, right-sized unit — a wave (§Wave below) — not on the epic.
...
**Surfaces:**
- `role:<name>` — a wave `selector` form alongside `tag:<name>` and `parent:<id>` (single parse point: `parse_wave_selector`, `brana-core/src/tasks/wave.rs`). `wave_pull_decision`'s eligibility check calls `derive_role(t) == Some(Role::ReadyForAgent)` directly — no second inline `ac_state`/`parked` predicate. A role-selector wave is a **standing wave** (ADR-086 §5, t-3161): its pull sorts candidates by priority then `created` (bespoke waves keep array order), defers tasks matched by any other *draining* `tag:`/`parent:` wave (`deferred` count in `NoneEligible`), and counts wip live as in_progress tasks that would derive the role were they pending (roles are status-derived, so a naive selector match on in_progress tasks is always empty) — semantics in [drain-loop.md §Standing wave](../../guide/workflows/drain-loop.md).
...
- **D2 — Auto-close:** chose *prompt on empty* ("epic empty; contract met? mark done") over silent auto-close, to avoid premature close when the contract carries criteria beyond "tasks done." **Decided per recommendation, 2026-07-20.** *2026-08-29 (t-3162): the "contract met?" question is now partially answerable by machine — `brana backlog wave ship <id>` evaluates the contract's `CHECK:` lines and shows the result before the human decides (see D5 amendment). Still a prompt, never an auto-close.*
...
- **D4 — WIP breach:** chose *warn (advisory)* over hard-block during the pilot, matching the existing spec-gate posture. **Promotion criterion (not open-ended):** reviewed alongside the spec-gate pilot (target 2026-07-28) — hard-block by default at review unless the pilot shows false-positive noise. **Decided per recommendation, 2026-07-20.**
  **RETIRED 2026-08-12 (t-2727), promotion review resolved.** The promotion review (t-2531) was cancelled unactioned past its 2026-07-28 target; t-2535 (2026-07-29) made the cap honor parking but left it advisory; t-2561 (2026-07-29) re-diagnosed the same warn-vs-block question plus a scoping defect (fires on every epic, not just the active one) and listed *retire* as one of three admissible outcomes. Two independent live incidents (2026-08-11, 2026-08-12) confirmed the warning had become pure alert fatigue — fired on every add, satisfiable by no action the warning itself named. Live measurement 2026-08-12: 9 of 55 epics in this project were over the default cap of 10, several by 4-7x (research-inbox 74, cli-backlog-schema 55, spec-docs 50, harness-core 39) — *retune* (the initially-favored option) would still have left the largest epics over cap. **Chosen: retire, not retune or enforce.** Rationale: epics are meant to be unbounded groupings ("what we're building" — see this doc's own Context above), not concurrency-limited; the cap was measuring the wrong unit. WIP/concurrency control, if wanted, belongs on **waves** instead (§Wave below) — a wave is already the right-sized, drainable, gated slice of work, and (per the wave-drain design, `wave drain` was conceived to let `/loop`/autonomous runners pick jobs off a wave's queue) a WIP cap there would bound *actual concurrent execution*, not an epic's total accumulated backlog size. Deferred to a follow-up (t-2782, blocked on `wave drain` landing first, t-2775) rather than designed now, to avoid repeating this same mistake — picking a limit with no usage data to size it from. `wip_limit` field and `check_epic_wip_cap()` deleted; `wip_limit` added to `RETIRED_FIELDS`. t-2561 resolved by this decision (RETIRE was one of its three admissible outcomes) — its `scope+enforce` blockers (t-2558/t-2563) are moot for this outcome.
- **D5 — Log vs conventions:** chose *one new `log` field* + `context` conventions (`CHECK:`, `ref:`) over multiple new fields, for simplicity / adopt-don't-build. **Decided per recommendation, 2026-07-20.**
  **Amended 2026-08-29 (t-3162, ADR-086 §6):** `CHECK:` now has two homes with different force — on task `context` it stays a *soft human reminder* (unchanged); on a **wave `contract`** it is *evaluated-and-shown* by `brana backlog wave ship <id>` against an allowlisted vocabulary only (task-state predicates, `selector count`, merge-base against a named branch, named `validate.sh --check N` / `cargo test -p <crate>` forms — parser: `brana-core/src/wave_ship.rs`). Display-only, never a gate: a red check informs the human and never blocks or causes the flip (ADR-080 §7 intact; guide L3.7 rung 1). Probe amendments honored: `CHECK-EXEMPT: t-NNN <reason>` carve-outs render as exemptions (A1); unparsed contract prose renders under "unevaluated — needs you" (A2). `wave set <id> contract` is runner-denied (`runner-verb-guard.sh`) so the gauge is not armed by the party it constrains.
  **Amended 2026-08-30 (t-3234):** the evaluator moved from `brana-cli` into `brana-core` (`wave_ship::build_ship_report`) so the MCP `backlog_wave_set` tool can call the same function — it used to flip `field:status value:shipped` with no evaluation at all, a second ship surface silently skipping the gauge. MCP's result now carries a `check_report` array with the same lines the CLI prints; still display-only, still never gates.
...
- **`--write` deferred during BUILD, then run 2026-07-23**: the migration script was written, tested, and committed without running `--write` during BUILD (confirmed with operator at t-2312 and reconfirmed at the BUILD→CLOSE gate) — then the operator ran it for real the next day, once its own stated precondition (t-2313's node-lifecycle code shipped) was satisfied. See "Migration executed" below.
...
- **`active_epic` resolution is backward-compatible**: accepts either a `type:"epic"` node (post-migration) or a flat `epic` tag match (pre-migration) — so nothing regressed while the migration sat unrun, and continues to hold no-op-safe now that it has run.
...
- **t-2312 (done, 2026-07-22).** Migration script `collapse-level-epic-v3.py` — level→type collapse (type wins on conflict, only 1 live conflict) and epic→node conversion (48 unique epic slugs measured live, drifted from the ADR's 43: 44 new `type:"epic"` nodes created, 4 pre-existing `in-*` initiative markers reused and retyped to `epic`). Deliberately does **not** re-parent the 714 tasks that already sit under a milestone/phase — 88 of their 166 distinct parents span more than one epic value, so epic is measurably orthogonal to the existing tree today. Operator confirmed accepting this design 2026-07-22; the mixed-epic-milestone reconciliation is tracked separately (t-2323). Dry-run verified against a full copy of the live 2,186-task dataset (idempotent on rerun). **Script landed with `--write` deliberately NOT run during BUILD** — operator chose to defer the actual migration until t-2313's node-lifecycle code shipped. **Run for real 2026-07-23** (`chore(backlog-v3): run collapse-level-epic-v3.py --write, fix epic marker status enum`, commit `4bd64233`): dropped `level` (1442 tasks) and `epic` (1869 tasks), created 46 epic-node tasks (reusing 4 existing `in-XXX` markers), re-parented 1119 previously-unparented tasks. The 4 reused markers had `type` normalized `initiative`→`epic` but kept task-style `status:in_progress`, which `validate_epic_status()` (t-2313) rejects — fixed to `status:active` via the rebuilt CLI in the same commit. Post-run integrity verified (2026-07-23 close): zero data loss, zero new orphaned `parent` refs (9 pre-existing orphans found predate the migration entirely), `validate.sh` Checks 62/63 clean. Two follow-up gaps filed, not fixed here: t-2322 (`validate_schema()`'s type whitelist doesn't include `epic`/`initiative`) and t-2323 (13 stray `type:"initiative"` tasks + the 88 mixed-epic milestones — still open, the migration did not touch already-parented tasks).
- **t-2316 (done, 2026-07-22).** Config cleanup: `active_initiative` dropped from `system/state/tasks-config.json` (project-local — the global copy never had it, confirming iteration-1's earlier claim was already stale); `github_sync.labels.stream` flipped to `false` in both the project-local and global `tasks-config.json` copies. Found during verification, not in the original task scope: a *second*, independent live consumer of `stream` labels — `brana-cli/src/sync.rs`'s `load_sync_config()` reads a wholly different file (`~/.claude/task-sync-config.json`) and hardcoded `label_stream: true` unconditionally, never consulting `tasks-config.json` at all. Fixed inline (flipped to `false`) since it directly matches the ADR's retirement intent and was a small, contained change. **Not yet done** (filed as follow-up, out of scope for this subtask): the flat `stream` field itself still carries values on 72 live tasks and has no write-path sealing (unlike `level`/`epic`) — see t-2325.
- **t-2313 (done, 2026-07-22).** Epic node lifecycle: `validate_epic_status()` (active/next/parked/done/archived — a separate function from `validate_status()`, dispatched by `task["type"] == "epic"` at every write surface: `set_field`, `cmd_add`, MCP `backlog_add`); `wip_limit` settable field (default 10 applied at read time, not stored); `check_epic_wip_cap()` — a single shared brana-core function (same pattern as `tag_matches()`, t-2311) wired into both `cmd_add` and MCP `backlog_add`, advisory warn-not-block per ADR-065 D4. Found and fixed during composition (not in the original task scope): `classify()`'s blocked-by gate hardcoded the task-status terminal vocabulary (`completed`/`cancelled`) and would have left an epic `blocked_by` another epic permanently stuck — an epic set to `status:"done"` never satisfied a dependent epic's gate. Fixed via a new `is_finished()` helper that recognizes `done`/`archived` as terminal specifically for `type:"epic"` tasks. Also fixed inline, both flagged as necessary rather than optional by the composing agent: `validate.sh` Check 26 (now type-aware — epic vocab vs task vocab — otherwise the feature breaks the blocking gate the moment any real epic status is set) and t-2312's migration script (`make_epic_node()`'s default status was `"pending"`, invalid epic vocab — changed to `"next"` before the script is ever run with `--write`). Also fixed, beyond the delegated report: both `cmd_add`'s and MCP `backlog_add`'s implicit status *default* (when no status is supplied at all) still hardcoded `"pending"` regardless of `type` — same class of bug as the migration script's, just via the live add paths instead. All fixes verified with the full 1117-test workspace suite green.
- **t-2314 (done, 2026-07-22).** `active_epic` fail-loud resolution, done in-session rather than delegated. `assert_active_epic_resolves()` (brana-core) checks whether the active epic slug resolves to EITHER a `type:"epic"` node task (post-migration shape) OR a task still carrying the flat `epic` tag (pre-migration compat — at write time the migration hadn't run yet, and the current live `active_epic` ("harness-core") resolved via 110 flat-tagged tasks, confirmed before wiring this in so it wouldn't regress `backlog focus`; the migration ran the next day, 2026-07-23, and this same backward-compatible check now resolves against the `type:"epic"` node instead, no code change needed). Wired into `cmd_focus` (CLI) and MCP `backlog_focus` — the only 2 confirmed live consumers (per iteration-1 challenger finding). Deliberately did NOT wire into `check_epic_wip_cap` (t-2313) — verified it doesn't consult `active_epic` at all, it operates on an explicit `parent_id` regardless of which epic is "active," so there was no natural wiring point; the original ISC wording ("...+ new WIP-cap check") didn't survive contact with the actual implementation.
- **t-2315 (done, 2026-07-22).** Wave storage object (D3): a `waves` sibling array in `tasks.json` (alongside `tasks`), NOT a separate file — precedent-driven decision: the "small sibling JSON store" pattern (`remind.rs`/`queue.rs`) is for global, cross-project, single-instance stores with their own lock; waves are project-scoped and operationally coupled to the same task tree, so they share `tasks.json`'s existing `load_raw`/`save_tasks`/`lock_tasks` atomicity boundary instead of introducing a second one. Shape: `{id: "wave-N", name, selector (opaque text, not parsed/executed), contract, gate, status: queued|draining|shipped, created}` — `wave-` id prefix deliberately distinct from `t-` (a separate array, separate numbering, so a bare "wave-3" and "t-3" can't be confused). `next_wave_id()`/`validate_wave_status()`/`set_wave_field()` in brana-core; `brana backlog wave add/get/list/set` CLI subcommands; matching `backlog_wave_add/get/list/set` MCP tools (registration was cheap enough to build full CRUD parity rather than a CLI-only partial). Status transitions are free (any-to-any), matching the precedent of `validate_status`/`validate_epic_status` — neither of those enforces forward-only ordering either, and the drain/query logic that would give "ordering" real operational meaning is explicitly out of scope for this slice. No referential check on `gate` (an unresolved/nonexistent wave id is accepted) — matches how `parent`/`blocked_by` are never existence-checked at write time elsewhere in this schema. Selector resolution, `wave drain`, and any query-execution engine remain explicitly deferred to the intent-CLI, a separate later build unit.
- **t-2317 (done, 2026-07-22).** This entry. User guide at [docs/guide/features/backlog-v3-schema.md](../../guide/features/backlog-v3-schema.md). Domain glossary (`docs/domain/MODEL-001-brana-core.md`) updated to add Epic as a domain entity.
- **t-2375 (done, 2026-07-23).** First consumer breakage found from the previous day's real `--write` run (t-2312 entry above): the close skill's Tier 2a/2b epic-detection cascade and `/brana:backlog start`'s branch-slug computation both still read the now-always-empty flat `epic` field, silently degrading to their fallback tiers (or a hard-stop with a dead remediation command, in start.md's case). Fixed both via a shared `resolve_epic_ancestor()` helper (`system/skills/_shared/epic-ancestor-walk.md`) that walks `parent` to the nearest `type:"epic"` ancestor. Also hardened against a second latent issue found in Challenger Gate review: the 4 reused `in-*` markers (t-2312 entry) carry full-sentence subjects, not slugs — the helper now rejects non-slug ancestor subjects rather than propagating them. A third sibling (Rust `TaskFilter.epic`, backing the MCP `backlog_query`/CLI `--epic` filter param) was found but not fixed here — different subsystem, requires a `cargo` rebuild — filed as t-2377.
- **t-2380 (done, 2026-07-24).** Investigated live "invalid JSON" parse errors on `brana backlog get`/`query --json` under concurrent sessions sharing a checkout. Root cause is NOT a gap in brana's own write path — `load_raw`/`save_tasks` already write via `write_atomic` (temp file + POSIX rename) under an exclusive `lock_tasks`/`lock_tasks_timeout` flock, and every CLI/MCP write call site was verified to route through it (no direct `fs::write` to `tasks.json` bypasses this). Atomic rename means a reader that already opened the file keeps reading the old inode to EOF, so brana's own writers cannot produce a torn read. The likelier source is an out-of-band writer that doesn't go through `write_atomic` at all — most plausibly `git checkout`/`merge` rewriting the working-tree file in place on a shared main checkout, which the worktree-only branching rule (t-2216/t-2206) already exists to reduce but can't eliminate for tools outside brana's control. Added a bounded defense at the read boundary instead of a lock (reads are deliberately unlocked for performance — see `backlog_get.rs`): `load_tasks`/`load_raw` now retry a JSON *parse* failure only (never an I/O error) up to 3 times with a 15ms backoff, re-reading the file fresh each attempt via a shared `read_with_retry()` helper in `brana-core/src/tasks.rs`, so a transient torn read self-heals once the racing writer finishes. No behavior change on the non-racing path. Full workspace test suite green (1000+ tests across all crates); proxy unit test added (`test_load_raw_retries_past_transient_parse_failure`) simulating a torn read fixed up by a concurrent thread within the retry window.

- **t-2765 (done, 2026-08-12).** t-2377's flat `TaskFilter.epic` had already been fixed (t-2740 wired `compute_stats`'s `by_epic` through `resolve_epic_ancestor`), but a 2026-08-12 audit found four more live consumers still reading the retired flat field directly, so they silently no-op'd for every real task: MCP `backlog_focus`'s active-epic boost, `session_initiative.rs`'s `maybe_write_initiative_marker` (session-epic marker never wrote, breaking close Step 9c Tier 1's detection), and — found by a requested repo-wide sweep, not in the original bug report — CLI `cmd_focus` (`brana backlog focus`/`brana focus`) had the identical boost bug at two call sites, completely untested. All four now resolve via `resolve_epic_ancestor`; `maybe_write_initiative_marker`'s signature changed to accept the full task list (was single-task, couldn't walk `parent`). `system/scripts/backlog-reconcile.sh`'s `--epic` filter and `by_epic` breakdown were also broken (always matched/bucketed nothing) — restructured into an age/priority pre-filter stage followed by per-survivor epic resolution via the canonical **bash** `resolve_epic_ancestor()` in `epic-ancestor-walk.md` (not a fourth Python reimplementation of the walk). Verified end-to-end against live data. Also fixed, found while reading the same file: a hardcoded `date(2026, 5, 30)` had silently frozen every age calculation. New permanent regression guard, `system/scripts/tests/test-no-flat-epic-field-reads.sh`, greps for the same bug shape across `system/` — makes the sweep durable instead of a one-time check.

- **t-2764 (done, 2026-08-12) — `backlog_query(epic:...)` resolution mechanism, traced end to end, source-cited.** No new mechanism; this closes the gap between "observed behavior" and "verified against the code that produces it," per a live confusion where `backlog_set(field: "epic")` was tried and rejected with `unknown field: epic`. No per-task field feeds epic membership — `resolve_epic_ancestor` (below) never reads a stored `epic` key on the task itself; `epic` reaches it only as a query-time **parameter**. (A narrower, separate legacy fallback that *does* still check for a stored `epic` key survives elsewhere — see the `active_epic` bullet.) Full chain, one function, three callers:
  - `resolve_epic_ancestor(task, by_id)` (`brana-core/src/tasks/query.rs:180-199`) walks `task["parent"]` up to 10 hops; at each hop, if the ancestor's `type == "epic"`, returns its `subject` — but only if `is_epic_slug(subject)` passes (`query.rs:163-167`, mirrors the bash regex `^[a-z0-9]+(-[a-z0-9]+)*$` in `epic-ancestor-walk.md`, rejecting the 4 pre-v3 `in-*` markers that carry full-sentence subjects). No match by the depth cap → `None` (a real "no epic ancestor"). **Correction (challenger gate, 2026-08-12):** an earlier draft of this entry claimed the Rust return type makes a dangling/corrupt `parent` id indistinguishable from a genuine "no ancestor" *impossible by construction* — false as stated. `by_id.get(id)?` (line 187) folds both cases into the same `None`; that ambiguity class is unchanged. What Rust actually eliminates is narrower: `epic-ancestor-walk.md`'s bash version can conflate "no epic" with "the `jq` subprocess failed to parse," because both silently print `""`. There's no subprocess in this path, so that specific conflation is gone — not general dangling-pointer detection.
  - `TaskFilter.epic: Option<&str>` (`query.rs:128-141`) is the query parameter carrying the requested slug. `filter_tasks_by` (`query.rs:202-265`) applies it at `query.rs:244-249`: for each candidate task, call `resolve_epic_ancestor`, keep the task only if the returned slug equals the filter's slug exactly. No tag, no membership list, no cache — computed fresh per query, per task, every call.
  - Three callers, one shared function, no divergent copies: CLI `cmd_query` (`brana-cli/src/commands/backlog.rs` — `TaskFilter { epic: epic.as_deref(), .. }` feeding `filter_tasks_by`), MCP `backlog_query` (`brana-mcp/src/tools/backlog_query.rs:60-73` — identical wiring), and `compute_stats`'s `by_epic` breakdown (`tasks/stats.rs:145`, fixed in t-2740). `brana backlog query --epic <slug>` and `mcp__brana__backlog_query(epic: "<slug>")` are the same code path through two transports, not two implementations.
  - **Distinct from `active_epic` — and NOT the same guarantee.** `assert_active_epic_resolves()` (`query.rs:110-124`, backs `backlog focus`/`backlog next`) is a *separate* function checking whether the single `tasks-config.json` `active_epic` pointer names something real — precondition validation on one pointer, not the per-task membership resolver above. **Correction (challenger gate, 2026-08-12):** it is not purely epic-node validation. Lines 117-120 keep a pre-migration compat branch — `all.iter().any(|t| t["epic"].as_str() == Some(active_epic))` — that accepts a task still carrying a flat `epic` tag matching the pointer. Live `tasks.json` today has zero tasks with a flat `epic` key (verified), so this branch is currently dead, but it is real code, not hypothetical: if an `epic` key were ever reintroduced by something that bypasses `set_field`'s `RETIRED_FIELDS` rejection (a raw edit, an old backup, an import script), `assert_active_epic_resolves` would silently accept it while `resolve_epic_ancestor` never would — reintroducing the exact per-function divergence this task exists to close out. Worth closing in a follow-up (drop the compat branch now that the migration has run), not fixed here since it's out of this task's docs-only scope.
  - **Live re-verification, not just a code read.** `mcp__brana__backlog_query(epic: "ruflo-integration")` returns 92 tasks as of this entry — up from the 65→77 range recorded in a prior session (2026-08-12, same day), which itself was after re-parenting 12 tasks under t-2362. The rising count on an unchanged epic node confirms the resolution is computed live against current `parent` pointers, not a cached or stored membership list — consistent with the mechanism above and inconsistent with any design that would need a backfill step when a task's `parent` changes.
  - `system/rules/task-convention.md`'s field list updated to add the v3 fields it was missing entirely (`work_type`, `acceptance_criteria`, `ac_state`) and to state `epic` is a derived query parameter, not a field — pointing here rather than restating the mechanism (same single-authority pattern as `branch-prefix.md`, t-2494).

### Plan vs. actual — deltas worth recording

- **Scope corrected mid-flight**: the original 10-subtask split had t-2312 (migration script) exhaustively deleting the flat `epic` field, then t-2313 (epic-node lifecycle) re-parenting tasks based on that same already-deleted value — a real sequencing bug caught before either was delegated. Fixed: t-2312 now owns both the level collapse AND the epic-to-node re-parenting; t-2313 is code-only (status/wip_limit/gate enforcement), not data migration.
- **Migration deferred during BUILD, run the next day.** `collapse-level-epic-v3.py` was written, tested against a full copy of the live dataset, and committed without running `--write` during BUILD. The operator ran it for real 2026-07-23 (commit `4bd64233`), once t-2313's node-lifecycle code (its own stated precondition) had shipped. Every write-path/read-path change (write-path sealing, `active_epic` resolution, epic-status validation, epic-node defaults) was deliberately built to tolerate BOTH the pre-migration shape (flat `epic` string) and the post-migration shape (`type:"epic"` node) — this paid off: nothing needed a code change when the migration actually ran. Post-run integrity independently re-verified at the next session's close (zero data loss, zero new orphans, Checks 62/63 clean).
- **Four follow-up tasks filed, not resolved here** (all tagged `follow-up`, epic `cli-backlog-schema`): t-2322 (type-enum whitelist missing `epic`/`initiative`), t-2323 (13 stray `type:"initiative"` tasks + 88 mixed-epic milestones needing human reconciliation), t-2325 (flat `stream` field itself never dropped — only its config-flag consumers were disabled), t-2377 (Rust `TaskFilter.epic` still filters against the retired flat field, backing the MCP `backlog_query`/CLI `--epic` param — found by t-2375's Challenger Gate).
- **Scope held**: the full 16-verb intent-CLI, AC backfill, and skills-as-loops (t-2278) stayed out of scope throughout, as decided at kickoff. `wave` and `epic` both landed as storage + minimal enforcement only, with selector/query resolution explicitly deferred.
- **Delegation pattern**: 7 of 9 implementation subtasks were composed by parallel agents (no worktree write access) and applied, tested, and verified by the coordinator against the live worktree — every delegated report was checked line-by-line against actual file content before editing, and running the full workspace test suite (not just the subset the delegate named) caught at least 3 real bugs delegation alone missed (a `cli_smoke.rs` regression, an off-by-one in a WIP-cap test, and two other write-surfaces defaulting to an invalid epic status).
