...
**BUILD→CLOSE challenger gate (2026-08-17, rung 1 — challenger + second-variant finder in parallel): PROCEED WITH CHANGES → fixed.** All 7 ISC criteria verified MET; no scope creep/miss; no security findings; SIBLINGS: no from the challenger, SIBLINGS: yes from the second-variant finder (below). Max severity 3, non-blocking, folded in directly:
- Challenger sev3: `loops-lint.py`'s denied-verbs check was substring-match on prose (not a real heading), and `supervised`'s value was never validated. Fixed: heading-anchored regex (`^#+\s*denied verbs`, case-insensitive) + `supervised` must be a real boolean, `false` explicitly rejected.
- Second-variant finder sev2 (structural sibling): ADR-079 §2b lacked the same "committed-doc addendum" ADR-080 §3 got for epic-drain in this same diff. Fixed: added the parallel addendum pointing at `docs/guide/workflows/drain-loop.md` + `system/loops/drain-loop.md`.
- Second-variant finder sev2: `loops-lint.py` type-checked `records` but not its structural siblings `drains`/`fills`/`spawns`. Fixed: all three now require list type.
- Challenger sev2 (not fixed, informational): new PyYAML dependency is unpinned/undocumented — bounded failure mode (loud traceback, not silent), no requirements file exists anywhere in this repo to add it to; left as-is.
- 6 new regression tests added (denied-verbs heading vs substring, supervised true/false/non-bool, drains/fills/spawns wrong-type) — 30/30 green; all 3 real entries re-verified lint-clean under the strengthened checks (no regression).
