...
## Consumer: the `ac-propose` loop (built by t-2288 — this slice unblocked it)
...
### As built (t-2288, 2026-07-21)

The loop is a **clean split**: the `brana` binary stays LLM-free (deterministic drain + scoped
persist), and AC *content* is generated by the agent layer (agy-first → Claude, per
`delegation-routing.md`). This keeps the mutation unit-testable while the wording comes from a real
generator.

- **Inert holding field:** `proposed_acceptance_criteria` (array of strings), deliberately **separate**
  from the live `acceptance_criteria` gating field. Promotion (`proposed → approved`, human) moves the
  array into `acceptance_criteria`; the array form makes that a lossless move. Const:
  `brana_core::tasks::PROPOSED_AC_FIELD`.
- **Drain (list):** `brana backlog ac-propose` (read-only) emits candidate packets
  (`id`, `subject`, `description`, `context`) straight from `tasks::ac_propose_candidates` — the filter
  is reused verbatim, never re-implemented.
- **Persist (apply):** `brana backlog ac-propose --apply <file|->` reads
  `[{"id","proposed_acceptance_criteria":[…]}]` and writes **only** `ac_state:proposed` +
  `proposed_acceptance_criteria`, **only for current candidates** (the candidate set is recomputed from
  the live slice, so a proposal targeting a legacy/non-candidate task is a silent no-op). `--dry-run`
  reports the applied set without writing. Core: `tasks::apply_ac_proposals` (scoped, in-memory,
  the AC#3 test) → `tasks::perform_ac_propose` (file-level, mirrors `perform_rollup`).
- **The loop** = `ac-propose` (list) → agent generates ACs → `ac-propose --apply`. A packaged
  re-invokable skill is deferred to the wave-4 cockpit (graduation tracked by **t-2278**).

Running it (agent orchestrates the three deterministic CLI steps):

```bash
# 1. Drain — get the candidate packets (read-only)
brana backlog ac-propose > /tmp/ac-candidates.json
...
#   → {"ac_propose":["t-…"],"action":"proposed"}
```

Opt a legacy task into management first if needed: `brana backlog set <id> ac_state none`.
