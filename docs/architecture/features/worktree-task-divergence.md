...
- **`brana` resolves the canonical `tasks.json` regardless of which worktree it runs from.**
  Verified 2026-07-29 (when `.claude/tasks.json` was still git-tracked — it is untracked since
  [ADR-091](../decisions/ADR-091-tasks-json-untracked-canonical-snapshot.md), 2026-09-04, and
  moves under `.git/brana/` per [ADR-094](../decisions/ADR-094-tasks-json-ledger-in-git-common-dir.md)):
  back then every worktree carried its own copy at a different inode, and this branch's copy
  was two hours stale — yet a read issued
