...
| [features/private-state-sync.md](architecture/features/private-state-sync.md) | Private state sync (t-3352): client/venture state kept out of this public repo, `tasks-portfolio.json` routed to the private brana-knowledge repo, validate Check 75 guard |
...
| [ADR-093](architecture/decisions/ADR-093-link-signal-enrichment-queue-scoring-layer2.md) | Link-signal enrichment — filling the backlog with pending tagged candidates is a pump filling a QUEUE (ADR-079's eligibility filter is the guard), not arming the `ac approve` valve; project-relevance scoring runs post-sync over `knowledge.db` via `VectorProvider::query`, never inside the ingest write (`KnowledgeStore` has no vector read-back); deferred Layer 2's queue shape fixed now — wave `wip_limit` cap (never null) + untouched-N-days → `link-signal-expired` + `cancelled` closer pump (operating law 2, t-2587) |
| [ADR-094](architecture/decisions/ADR-094-tasks-json-ledger-in-git-common-dir.md) | Move the live `tasks.json` ledger out of the working tree into `$GIT_COMMON_DIR/brana/` — git clobbers *ignored* files on checkout to any ref that tracks the path (145 tags/branches still do), which wiped the ledger during the 2026-09-07 ship; also: fail-loud instead of auto-created empty ledgers, per-write backup rotation, the main checkout stays on `dev` forever, post-checkout restore backstop, non-overridable Gate 3 finding class (amends ADR-091 decision 1, extends ADR-060) |
...
| [drained/](ideas/drained/) | 74 idea docs converted to tasks or absorbed (incl. brana-efficiency-without-power-loss.md — all proposals shipped elsewhere) — notable: [link-signal-enrichment-pipeline.md](ideas/drained/link-signal-enrichment-pipeline.md) (t-3306..t-3314, 2026-09-06), [wave-pipeline.md](ideas/drained/wave-pipeline.md) (superseded 2026-08-23 → [the-brana.md](architecture/the-brana.md), D3/t-3028), [wave-pipeline-infographic-prompt.md](ideas/drained/wave-pipeline-infographic-prompt.md) (t-2867), [loops-library.md](ideas/drained/loops-library.md) (t-2826), [loop-first-redesign.md](ideas/drained/loop-first-redesign.md) (epic t-2820), [skills-as-loops.md](ideas/drained/skills-as-loops.md), [brana-whatsapp-agent.md](ideas/drained/brana-whatsapp-agent.md), [brana-agency-growth-machine.md](ideas/drained/brana-agency-growth-machine.md) (t-2249) |
