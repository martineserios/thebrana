...
**Verdict: Keep as capability.** Adversarial review (Sonnet challenger) is valuable for big decisions. Implemented as `/brana:challenge`. Subscription-native, rate-limit-aware. Low maintenance, high value when used. Model changed to Sonnet for cost-effective adversarial review (see [13-challenger-agent.md](../../../brana-knowledge/dimensions/13-challenger-agent.md)).
