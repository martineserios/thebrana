...
**Skill-routing gate: domain skills require confirmation before loading.** `system/rules/work-start.md` §4 mandates that every work session must identify two skill layers — a workflow skill (HOW: build, research, align) and a domain skill (WHAT rules: rust-skills, etc.) — and present both to the user via AskUserQuestion before any skill is loaded. Silent routing is explicitly forbidden. When build.md's LOAD step discovers a matching domain skill (score ≥ 0.5), it applies this gate rather than silently suggesting the skill inline. LOAD is the information source (which skills matched); work-start.md §4 owns the confirmation gate. This is enforced by the `work-start.md` rule, not a hook — rule-based enforcement was chosen over hook-based because hooks fire on every event (hot path overhead) while a rule is loaded once and applied in context. See `system/rules/work-start.md` and erratum #127.

**Skill cull telemetry closes the count-drift loop.** Brana has accumulated 27+ skills; every skill adds tokens to the system prompt. Zero-invocation skills are pure overhead. `brana skills usage` (ADR-035, Apr 2026) scans `~/.claude/projects/*.jsonl` for skill invocation events over a rolling 30-day window, producing a ranked list with last-used dates. Threshold: <5 invocations in 30 days = cull candidate. This provides evidence-based pruning — context budget decisions are now data-driven rather than impression-driven. Secondary signal: if `/brana:do` is intended to route freeform input but direct skill invocations dominate (>90%), the semantic router is not functioning.
