...
**Skill-routing gate: domain skills require confirmation before loading.** `system/rules/skill-routing.md` mandates that every work session must identify two skill layers — a workflow skill (HOW: build, research, align) and a domain skill (WHAT rules: rust-skills, etc.) — and present both to the user via AskUserQuestion before any skill is loaded. Silent routing is explicitly forbidden. When build.md's LOAD step discovers a matching domain skill (score ≥ 0.5), it applies this gate rather than silently suggesting the skill inline. LOAD is the information source (which skills matched); skill-routing.md owns the confirmation gate. This is enforced by the `skill-routing.md` rule, not a hook — rule-based enforcement was chosen over hook-based because hooks fire on every event (hot path overhead) while a rule is loaded once and applied in context. See `system/rules/skill-routing.md` and erratum #127.
