...
- **Context budget** — always-loaded context is gated as **two independent pools** (t-2505): AUTHORED (CLAUDE.md + `always-load: true` rules) under 22528 bytes, and ROUTING METADATA (skill + agent descriptions) under 10240 bytes. Both are computed by `system/scripts/context-budget.sh`, called by `validate.sh` Check 5 and the `pre-commit` budget gate. The split exists because a single cap let auto-grown routing metadata (~30% of the budget, added by nobody's decision) crowd out hand-written rules until only 19 bytes of headroom remained. Every KB competes with working context ([21-anthropic-engineering-deep-dive.md](../../../brana-knowledge/dimensions/21-anthropic-engineering-deep-dive.md)). **Critical caveat (from [35-context-engineering-principles.md](../dimensions/35-context-engineering-principles.md)):** the ~28KB rule budget validates the *controllable* variable — but it's not the dominant source of context loss. MCP tool definitions consume 30-70K tokens per session (reduced ~85% by Tool Search), and the compaction buffer reserves 33-45K tokens before auto-compaction fires. Total fixed overhead is 76-138K tokens (38-69% of the 200K window) — far larger than the rule budget. Validating that rules stay under 28KB is correct but incomplete: the assurance question is whether Tool Search is active and whether MCP server count is bounded.
...
- **Pre-commit validation** — `system/scripts/git-hooks/pre-commit` (tracked source; deployed globally by `bootstrap.sh`) validates two constraints before commit: attribution trailers (no Co-Authored-By, no AI attribution) and context budget (blocks when either gated pool exceeds its limit — AUTHORED 22528 B, ROUTING METADATA 10240 B). The budget gate is the shift-left complement to `validate.sh`'s deploy-time Check 5 — if budget grows silently between deploys, this catches it at commit. See [35-context-engineering-principles.md](../dimensions/35-context-engineering-principles.md) for budget failure modes
...
| `pre-commit` (budget check) | `git commit` | Committing when either gated pool is over — AUTHORED (CLAUDE.md + `always-load` rules) above 22528 B, or ROUTING METADATA (skill + agent descriptions) above 10240 B. Catches bloated rules or descriptions before they reach the repo, and stops one pool masking the other. Source: `system/scripts/git-hooks/pre-commit`. Runs only in brana repos (`system/skills/` + `system/hooks/` present). |
