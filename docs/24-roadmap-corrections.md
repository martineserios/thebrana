...
|---|---|---|---|---|
| E2026-05-08-1 | thebrana: `plugin-packaging.md` E1 resolution attributed the fix to a CC version upgrade — actual invariant is that CC reads `hooks.json` once at session startup; mid-session injection is silently ignored regardless of version | **Low** | pending | Fix: add lifecycle note to plugin-packaging.md: "Hook registration is read-once at CC session startup. Modifying hooks.json mid-session has no effect — restart CC to pick up new hooks." Also add restart reminder to bootstrap.sh output. 2026-05-08. |
