...
| E2026-06-15-4 | thebrana: `system/skills/challenge/SKILL.md` frontmatter `allowed-tools` listed `hive-mind_spawn`, `hive-mind_consensus`, `hive-mind_shutdown` but omitted `mcp__ruflo__hive-mind_init`. The `_shared/adversarial-hive-mind.md` shared procedure (created t-2106, 2026-06-15) calls `hive-mind_init` as its first setup step. `brainstorm/SKILL.md` correctly includes it. `challenge/SKILL.md` did not. Any invocation of the standard or `--hats` mode in challenge would fail silently or error on `hive-mind_init` at runtime. Root cause: `_shared/` extraction did not include a post-extraction allowed-tools audit sweep against consumer SKILL.md files. | **Medium** | code-fix (2026-06-15) | Added `mcp__ruflo__hive-mind_init` to `challenge/SKILL.md` allowed-tools (after `hive-mind_shutdown`, before `mcp__brana__agy_delegate`). See pattern `shared-procedure-allowed-tools-audit_2026-06-15` for the recurring prevention pattern. |

| E2026-06-15-3 | thebrana: `docs/architecture/field-notes/fts5-memory-recall.md` §Recall provider model states "Two providers: FTS5 (`brana memory search`): keyword, fast, ruflo-free; ruflo (`brana knowledge search`): semantic/vector, requires ruflo running." The brainstorm session 2026-06-15 designed a third impl — HybridProvider (parallel dispatch + RRF merge) — captured in `docs/ideas/drained/hybrid-provider-fts5-ruflo-rrf.md` and noted in t-2091 context. The field note is already stale as an architectural inventory. Any reading of that doc as the authoritative provider list misses the hybrid design and affects t-2091 scoping. | **Low** | applied (2026-06-15) | Field note added to `fts5-memory-recall.md` §Field Notes: "Planned third SearchProvider impl — HybridProvider (parallel + RRF)" with reference to `docs/ideas/drained/hybrid-provider-fts5-ruflo-rrf.md`. Applied in close session 2026-06-15. |

| E2026-06-15-2 | thebrana: t-2095 was marked `status: in_progress` in commit `df82a998` then completed via 3 refactor commits, but `brana backlog get t-2095` shows `"started": null, "completed": null` despite `"status": "completed"`. The lifecycle contract in `task-convention.md` expects `started` to be written on the `pending → in_progress` transition and `completed` on the `in_progress → completed` transition. Both are null. Velocity and cycle-time metrics derived from these fields are silently wrong. The `/brana:build` CLOSE step or the `brana backlog done` command failed to write the dates. | **Low** | applied (2026-06-15) | `system/skills/build/phases/close.md` step 7 updated to include explicit commands: `brana backlog set {id} status completed` and `brana backlog set {id} completed $(date +%Y-%m-%d)`. t-2095 dates manually fixed: started=2026-06-15, completed=2026-06-15. |

| E2026-06-15-1 | thebrana: `system/skills/backlog/SKILL.md` line 62 (Initiative Model v3 table) lists valid `work_type` values as `implement / research / design / ops / review / refactor`. The CLI enforces `implement / research / design / infra / chore / review`. The values `ops` and `refactor` are in the spec but rejected at runtime; the values `infra` and `chore` are in the CLI but missing from the spec. Setting `work_type: refactor` on a refactor-kind task (the obvious mapping) produces `invalid work_type "refactor"`. Correct mapping for refactor tasks is `work_type: implement`. Discovery: t-2095, 2026-06-15. | **Low** | applied (2026-06-15) | Fixed `system/skills/backlog/SKILL.md` line 62: replaced `ops / refactor` with `infra / chore`. Added note that `kind: refactor` tasks use `work_type: implement`. |

| E2026-06-10-6 | thebrana: `system/procedures/backlog.md` (and t-1950 migration runbook) implicitly use `deleted` as a valid task status for superseded or extracted tasks — e.g. "mark moved tasks as deleted". The brana CLI rejects `deleted` at runtime: `{"ok":false,"error":"invalid status \"deleted\" — must be pending/in_progress/completed/cancelled or null"}`. The correct status for superseded/migrated tasks is `cancelled` with a context note. Any procedure using `deleted` as a status will fail mid-execution. | **Low** | applied 2026-06-10 | Replace all `deleted` status references in `system/procedures/backlog.md` and any migration runbooks with `cancelled`. Add a field note: "cancelled is the terminal status for superseded/migrated tasks; use context --append to explain why." |

| E2026-06-10-5 | thebrana: `system/procedures/challenge.md` (challenger gate C4 finding) incorrectly states that `git-filter-repo` requires origin to be removed before running because "it refuses to run on clones with a configured remote". Reality: `git-filter-repo` uses origin *presence* as its freshness proxy — it removes origin itself as part of its operation. Removing origin first causes the tool to refuse with "Refusing to destructively overwrite repo history since this does not look like a fresh clone." The correct fallback is `--force`, not pre-removing origin. Following C4 as written would abort every history extraction. | **Medium** | applied 2026-06-10 | Correct the C4 finding in `system/procedures/challenge.md` (or equivalent challenger knowledge base): "Do NOT pre-remove origin before running git-filter-repo. filter-repo removes origin itself. Use --force only as a fallback if filter-repo still refuses on a verified throwaway clone." |

| E2026-06-10-4 | proyecto-anita/dgrx: `settings.py` line 77 field `esteban_personal_wa` is stale — Esteban Bosch left 2026-06-01; field was repurposed for Mili (Milagros Huayta) with a comment but never renamed. Any code grepping for `esteban` will incorrectly find this field; any new dev unfamiliar with history will be confused. Deploy env vars that set this key also carry the stale name. | **Low** | pending | Rename `esteban_personal_wa` → `mili_personal_wa` in `settings.py`, `deploy-dgrx.sh` `--update-env-vars`, and any other references. Update `CLAUDE.md` vendedores table. |

| E2026-06-10-3 | thebrana: `docs/architecture/skills.md` lines 26, 30, and 61 describe the Extended-skill procedure-loading path as `system/procedures/{name}.md` (absolute from repo root). After t-1931 (2026-06-10), all 33 SKILL.md stubs were changed to use base-dir-relative `../../procedures/{name}.md`. The architecture doc still shows the old form; anyone authoring a new Extended skill following the doc would write the wrong path and fail procedure resolution. | **Low** | applied (2026-06-10) | Updated `docs/architecture/skills.md` lines 26, 30, and 61 to show `../../procedures/{name}.md` with note that this form works in both repo and plugin layouts. |

| E2026-06-10-2 | thebrana: `system/rules/task-convention.md` (and `~/.claude/rules/task-convention.md`) document task status transitions (pending/in_progress/completed/cancelled) but say nothing about what happens to child tasks when a parent is cancelled. Cancelling t-719 (2026-04-04) left children t-724–t-729 as live pending tasks that survived for 2+ months, polluting the backlog and inflating pending counts. No automated or documented sweep existed. | **Low** | applied (2026-06-10) | Added to `system/rules/task-convention.md`: "Cancelling a parent task does NOT auto-cancel children. When cancelling a parent, manually cancel or re-parent all children. `brana backlog tree <parent-id>` shows the subtree." Bootstrap.sh propagates to `~/.claude/rules/`. |

| E2026-06-10-1 | thebrana: `docs/ideas/session-aware-loop-integration.md` line 45 states "CronCreate is main-context only — skills can't invoke it (CC platform constraint as of v2.1.87)." This is false. Empirically dissolved 2026-06-09: a cron job (`20a4a021`) was created and deleted from within a `brana:backlog` skill flow without issue. Skill procedures execute in main context. The false constraint directly caused initiative t-719 Phase 3 to be shelved as infeasible for 2+ months. Six child tasks (t-724–t-729) survived as zombie pending tasks against a non-existent blocker. Lines 54 and 74 of the same doc also carry this false assumption. ADR-050 explicitly dissolves the constraint in §Context. | **Medium** | applied (2026-06-10) | Updated lines 45, 54, 74 of `docs/ideas/session-aware-loop-integration.md` to reflect dissolved constraint with ADR-050 reference. |

| E2026-06-09-3 | thebrana: `system/procedures/close.md` ToolSearch preamble (line 20) covers only `mcp__ruflo__*` tools. The procedure body references `mcp__brana__memory_index` (Step 11 overflow pre-pass, prose and implied call). `validate.sh` Check 45 flags this as a missing ToolSearch registration on every run, failing the check. The tool is deferred and won't resolve via `ToolSearch` at runtime unless declared in the preamble. | **Low** | pending | Add `mcp__brana__memory_index` to the `ToolSearch("select:...")` call at line 20 of `system/procedures/close.md`. |

| E2026-06-09-2 | thebrana: `system/procedures/challenge.md` step 4b "On error" clause matched only responses where `response starts with "Error:"` — a string-pattern guard for application-level errors. When `mcp__brana__agy_delegate` fails at the MCP transport layer (version mismatch, binary not found, JSON-RPC error), the caller receives a tool-call exception, not a string response. The string-pattern guard never fires; the `agy_delegate` version pin error (`pmcp::Error::validation`) propagated up uncaught, causing the challenger to silently have no Gemini constraints with no indication of why. The distinction: application errors → response body starts with "Error:"; transport errors → tool invocation itself throws. Step 4b only defended against the first class. | **Medium** | code-fix (2026-06-09) | Fixed in commit `e4e35b01` (t-1922): broadened step 4b "On error" clause to cover all failure modes: `(a) MCP tool-call failure (version mismatch, binary not found, JSON-RPC error), (b) response starting with "Error:", (c) empty/unusable output`. All three paths now trigger the silent-skip path. |

| E2026-06-09-1 | thebrana: `docs/architecture/agents.md` Frontmatter Field Reference table documents `skills:` as `Domain skills to preload at agent spawn — injects their SKILL.md body as preloaded context`. This is incorrect: `skills:` is not a native CC frontmatter field — CC silently ignores it. An agent author adding `skills: [rust-skills]` will see no injection and no error, making it appear the agent just "doesn't have the skills loaded" for unclear reasons. The correct mechanism is an explicit `## Preloaded Knowledge` section in the agent body, where skill content is embedded directly. Discovered when challenger.md was found with `skills:` frontmatter that was having no effect (t-1215, 2026-06-09). | **Medium** | code-fix (2026-06-09) | Removed `skills:` row from Frontmatter Field Reference table; added callout paragraph and field note pointing to `## Preloaded Knowledge` body section. `system/agents/challenger.md` and `pr-reviewer.md` updated. Committed `a9921f63`. |

| E2026-06-08-12 | thebrana: `validate.sh` Check 31a warn/cap thresholds were calibrated at warn=40/cap=50 but `~/.claude/memory/patterns.md` had grown to 77 entries across multiple sessions. The check's auto-prune mechanism removes oldest quarantine entries to hold the file at cap, silently discarding valid patterns rather than flagging that the threshold was too low for the project's actual accumulation rate (~10–20 patterns/session). The cap acted as a pruner, not a gate — patterns were lost rather than the threshold being questioned. | **Low** | code-fix (2026-06-08) | Raised to warn=80/cap=100 in `validate.sh` Check 31a (t-1909). The file's own `<!-- cap -->` comment was also updated. Review thresholds when the file crosses the warn level rather than waiting for auto-prune to fire. |
...
| E2026-06-08-2 | thebrana: `close.md` Step 11 (Memory Review) and the `/loop` maintenance sweep trim MEMORY.md by removing index lines. The `mcp__brana__memory_index` tool (called by the `SessionStart` hook) performs a filesystem scan — not a diff against the existing index. Any `.md` file present in the memory directory is re-added to the index. Removing an index line without deleting the corresponding file is a no-op that is silently reversed on the next session start. Trimming 22 entries at loop tick N resulted in re-inflation back to 201 lines at tick N+1. | **Low** | applied (2026-06-09) | Both the close.md memory-trim steps and the loop maintenance sweep should state: "Trim requires two steps: (1) `rm` the `.md` file from disk; (2) remove the index line. Index-only edits are reversed by `memory_index` at next session start." Tracked: add note to loop.md §Knowledge Hygiene and close.md §Step 11 overflow pre-pass. |

| E2026-06-08-1 | thebrana: Hook commands in `system/hooks/hooks.json` referenced `${CLAUDE_PLUGIN_ROOT}/hooks/*.sh` — paths inside the git working tree. Git branch switches and `git stash` can remove these files from the working tree, causing "No such file or directory" errors for ALL hooks in every project sharing `CLAUDE_PLUGIN_ROOT`. Pre-existing drift confirmed: `pre-compact.sh` was absent from `~/.claude/hooks/` (had drifted silently). Root cause: no stable deploy target existed outside git; the deploy step (`make hooks-deploy` / bootstrap.sh sync) was manual and easily skipped. The Stop hook failure that triggered this fix (`goal-completion.sh: No such file or directory`) was a symptom of this structural problem. | **High** | code-fix | Fixed in commit `11b51c1e`: all hook commands migrated from `bash ${CLAUDE_PLUGIN_ROOT}/hooks/X.sh` to `bash "$HOME/.claude/hooks/X.sh"`. Added `Makefile` with `hooks-deploy` (rsync -a --delete) and `hooks-check` targets. Added `hooks-auto-deploy.sh` PostToolUse hook to auto-sync on Write/Edit to hook files (main branch only; retains `${CLAUDE_PLUGIN_ROOT}` as only exception). Added `branch-checkout-warn.sh` PreToolUse hook to nudge toward worktrees. |
...
| E2026-05-15-3 | thebrana: `brana backlog set` array-field syntax undocumented — `[]` is rejected; use `+val`/`-val`; negative values need `--` separator (`brana backlog set t-NNN blocked_by -- -t-MMM`) | **Low** | applied (2026-06-09) | Update `feedback_brana-backlog-set-positional.md` with array-field semantics |
| E2026-05-15-2 | thebrana: `branch-verify.sh` reverse-direction parent-dir match (`case "$bpath" in ${file}|${file}/*`) flags bare dir name `system` as behavioral — false positives on `.claude/tasks.json` + `docs/ideas/*` staging; also triggers when a Bash command payload contains the substring `git add` followed by the word `system/` | **Medium** | applied (t-1424 completed, 2026-05-15) | Fix: remove reverse-direction case (line 82-88) — actual blob paths from staging always include full subpath. Add regression test asserting `.claude/tasks.json` + `docs/ideas/foo.md` pass `is_behavioral()`. Track: t-1424 |
...
| E2026-05-14-3 | thebrana: `validate.sh` golden-path drift block (line ~1320) still labels itself "Check 27: Golden-path drift..." — duplicate of the real Check 27 (MCP wrapper exec pattern) introduced earlier | **Low** | applied (t-3168, 2026-08-24) | Renamed to "Check 73" (next number after the highest counted header) rather than this row's originally-suggested "Check 29" — that number was claimed by a different check (reference docs up to date) by the time t-3168 landed. Regression test added: `tests/procedures/test-validate-golden-path-check-number.sh`. t-1916 cross-closed. |
...
| E2026-04-30-5 | spec-first hook glob `*.test.*` does not match `tools/*.test.sh` paths on feat branches | **Low** | task filed (t-1917, 2026-06-09) | Workaround: write test artifact via Bash (not Edit/Write), then `git add` before subsequent edits. Root cause: hook matcher may check only `services/` and `tests/` prefixes. Needs audit of pre-tool-use.sh glob patterns for `tools/` coverage. Affected: `.claude/hooks/` (pre-tool-use.sh glob pattern) |
...
> **2026-08-08 note:** the field was since renamed to `epic` (t-1613/t-1614) and then retired entirely (ADR-065, write-sealed t-2310) — epic membership is now set via `parent` to a `type: "epic"` node; neither `initiative` nor `epic` is settable today.
