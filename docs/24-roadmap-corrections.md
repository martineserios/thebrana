...
| E2026-05-28-4 | nexeye: `supabase/migrations/20260528000001_camera_onvif_credentials.sql` comment says `-- encrypted at rest via Supabase Vault` but `onvif_password` column is plain `text` — no Vault integration exists. API route writes raw password string directly. Any DB read exposes plaintext credentials. | **Medium** | pending | Fix before first customer camera onboarding: either implement `pgsodium`/Vault encryption or replace comment with accurate "stored plaintext — encrypt before production". Route: `services/web-next/app/api/cameras/[id]/onvif-credentials/route.ts`. |
