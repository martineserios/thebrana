...
| E2026-05-08-6 | proyecto_anita: `templates` table has no unique constraint on `(company_id, name, language_code)` — ETL loader `ON CONFLICT (company_id, name, language_code) DO NOTHING` fails; `content` is NOT NULL with no default | **Low** | pending | Migration needed: `CREATE UNIQUE INDEX templates_tenant_name_lang_uq ON templates(company_id, name, language_code)` + ensure `content` has default or loader supplies value. Affected: `tools/anita-v2/_loader/`, `tools/migrations/` |
