...
| 118 | CLAUDE.md line 92 lists retired MCP wrappers as active | **Low** | pending | Spec says wrappers: `ruflo-mcp.sh`, `context7-mcp.sh`, `linkedin-mcp.sh`. Reality: only `ruflo-mcp.sh` is active. `brana-mcp` uses a direct binary (no wrapper). `context7-mcp.sh` and `linkedin-mcp.sh` are dead scripts — servers retired per ADR-036. Fix: update line 92 to list only `ruflo-mcp.sh`; note brana-mcp uses direct binary; note google-sheets is pinned in `~/.claude.json` (global). |
| 119 | SECURITY.md line 14 lists stale MCP server names | **Low** | pending | Spec says "claude-flow, context7, notebooklm". Reality: current stack is `ruflo` (formerly claude-flow), `brana-mcp`, `google-sheets`. context7 retired (ADR-036). notebooklm was never an MCP server. Fix: replace with current server list from `.mcp.json` + `~/.claude.json`. |
