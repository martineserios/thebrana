...
| E2026-05-28-3 | proyecto_anita/palco: H-7 — `idArticulo` (int) in `/articulos/` vs `codArticulo` (string) in `/stock/` — join key relationship unconfirmed. If they are separate code systems, all mart joins (M1 ABC, M2 ventas, M3 PDF) are wrong. Currently assumed `codArticulo = str(idArticulo)` but not validated with a live cross-check. | **Medium** | pending | Next Chess probe session: take a `codArticulo` from `/stock/` response and call `/articulos/?articulo=<value>`. If it returns a record, the fields are compatible and a simple cast suffices. If not, escalate to TCP/IT Palco. Tracked as H-7 in `chess-api-endpoints.md`. |
