...
| E2026-06-01-2 | thebrana: Any procedure that calls a deferred MCP tool (`mcp__brana__*` or other non-default tools) must include it in that procedure's ToolSearch preamble — not just the ruflo tools. The preamble was treated as "load ruflo" rather than "load all deferred tools this procedure touches." `challenge.md` and `research.md` both failed this way. Other procedure files may have the same gap. | **Low** | pending | Audit all `system/procedures/*.md` files: for each MCP tool called in the procedure body, verify it appears in the ToolSearch preamble. Run: `grep -l "mcp__brana__agy_delegate\|mcp__brana__backlog\|mcp__brana__session" system/procedures/*.md` and cross-check against their preamble lines. |
