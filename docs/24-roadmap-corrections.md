...
| 124 | `generate-reference.py` not wired as post-commit hook — reference docs drift on every hooks.json change | **Medium** | pending | Root cause of e87: no automatic trigger. Every manual hooks.json edit silently diverges docs/reference/hooks.md until manually re-run. Fix: wire as PostToolUse hook on Edit/Write targeting hooks.json, or add to post-commit hook. Task: t-1176. |
