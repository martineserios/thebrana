...
| E2026-04-30-1 | Agent v4 `build_conversation_context` relied on `execution_context.metadata` that Kapso never populates on real inbound | **High** | code-fix | t-538 dual-read bridge implemented in `build-conversation-context.js`: resolve_contact is authoritative; metadata is last-resort fallback. Comment added line 194 confirming the invariant. Doc fix still open: `docs/agent-v4/tool-contracts.md` + `architecture.md` should document that execution_context.metadata is never reliable on pure inbound. |
...
| E2026-04-30-3 | `external_customer_id` in Kapso System Variables but NOT propagated to function body Context Variables | **High** | code-fix | Triple-source resolution implemented in `build-conversation-context.js`: `body.input.external_customer_id ?? body.customer.external_customer_id` with fallback chain. Confirmed resolving on real inbound (t-538). Architecture doc (`docs/agent-v4/architecture.md`) metadata section still references old single-source assumption — doc update pending. |
