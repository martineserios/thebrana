...
| E2026-05-08-8 | thebrana: `docs/architecture/extending.md` deploy cycle section still references `deploy.sh` as the deploy command — `deploy.sh` is deprecated since v0.8.0 (prints deprecation notice and exits). Actual deploy path: `--plugin-dir ./system` (dev) + `./bootstrap.sh` (identity). | **Low** | pending | Fix: update extending.md deploy cycle section to remove deploy.sh references and describe the two-layer model. Reference developer-quickstart.md and bootstrap.md (both added 2026-05-08). |
