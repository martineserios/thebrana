...
|---|---|---|---|---|
| E2026-05-07-6 | proyecto_anita: `getAdminJwt` in `tracy-admin-auth.js` calls `env.KV.get()` without null-checking `env.KV` — throws TypeError instead of degrading gracefully when KV binding absent | **Low** | pending | Latent bug: production KV outage or cold-start without binding → uncaught TypeError, not a soft skip. Tests must use "partial KV" workaround (reads work, writes throw) to exercise write-failure degradation. Fix: add `if (!env?.KV) return null` guard at top of `getAdminJwt`. Affected: `services/kapso-functions/src/lib/tracy-admin-auth.js` |
...
| E2026-05-07-4 | proyecto_anita: `build-conversation-context.js` uses `marketplace_distributor_id` 271/272 (Palco/PDB) instead of correct values 52/53 per Greencode Tracy | **High** | pending | Wrong distributor scoping on every conversation — Tracy product lookups return data for wrong distributor. Source: Tommy Shilton confirmed 2026-05-07. Backlog t-737 (P0). Affected: `services/kapso-functions/src/build-conversation-context.js` lines 104, 133 |
