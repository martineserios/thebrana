...
| E2026-04-30-1 | Agent v4 `build_conversation_context` relied on `execution_context.metadata` that Kapso never populates on real inbound | **High** | pending | `execution_context.metadata` is only populated by broadcast pipeline pre-sync; pure inbound always `{}`. Spec assumed it was reliable. Fix: t-538 dual-read bridge (resolve_contact v3-api as authoritative source). Affected: `docs/agent-v4/tool-contracts.md`, `architecture.md`, ADR-025 |
...
| E2026-04-30-3 | `external_customer_id` in Kapso System Variables but NOT propagated to function body Context Variables | **High** | pending | `build_conversation_context.js` reads `body.customer?.external_customer_id` but `customer` is absent from Context Variables. Causes MISSING_EXTERNAL_CUSTOMER_ID — current Agent v4 blocker. Fix: add input_mapping to flow node OR extend triple-source resolution. Task: t-544 (architecture doc), new unblock task needed |
...
| E2026-04-30-6 | `deploy-multitenant.sh` uses `--clear-secrets` which silently drops ALL Secret Manager bindings (incl. `JWT_SECRET_KEY`) on every deploy | **High** | pending | Every staging + prod deploy of `palco-v3-api-*` strips the `agent-jwt-signing-secret` binding, breaking JWT verification until manual `gcloud run services update --update-secrets` is re-run. Directly violates `jwt-secret-discipline.md` Rule 3. Fix: replace `--clear-secrets` with `--update-secrets=JWT_SECRET_KEY=agent-jwt-signing-secret:latest` in the script; add post-deploy assertion that lists secret bindings on the new revision. Affected: `deploy/gcp/cloud_run/deploy-multitenant.sh`, `.claude/rules/cloud-run-deploy.md` (gate note) |
...
| E2026-04-20-8 | [Doc 14](reflections/14-mastermind-architecture.md) missing Knowledge Graph Discipline synthesis — doc 48 marked "Source for R2 graph architecture" but doc 14 only has one passing citation | **Medium** | pending | Add spec-graph.json governance subsection: two-tier extraction, ontology-constrained validation, schema evolution, JSON-vs-DB tradeoff justification. Source: [48-knowledge-graph-architecture.md](../../../brana-knowledge/dimensions/48-knowledge-graph-architecture.md) |
| E2026-04-20-9 | [Doc 32](reflections/32-lifecycle.md) missing Temporal Batching (two clocks) from doc 49b — fast clock (per-skill capture) absent from maintenance cadences | **Low** | pending | Explain how per-skill extraction (fast clock) feeds into weekly consolidation (slow clock). ADR-027 mentions the loop but doc 32 never connects the cadences. |
| E2026-04-20-10 | [Doc 32](reflections/32-lifecycle.md) missing Tiered Access + pruning SOP from doc 49b — knowledge tier is static (type-based), not access-frequency-driven | **Low** | pending | Add: access-frequency-driven promotion/demotion, and pruning workflow (flagged → archived → deleted) to connect decay detection to decay automation. |
