...
**Entities:**
- `Task` — id, subject, description, status, priority, effort, stream, type, tags, parent, blocked_by, branch, context, notes, created, started, completed, order, execution, github_issue, build_step, strategy; plus two nullable pump-lifecycle fields (ADR-080 §5, t-2841): `lease` `{claimant, expires}` — written only by `wave pull` in its lock critical section, cleared by ANY status write (key removed, never null); `reclaim_count` (int) — incremented by the future lease-reclaimer, survives lease clearing, removed on completion
