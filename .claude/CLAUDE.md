...
### 2026-04-14: Use `git -C <worktree>` not `cd <worktree> && git` in cross-repo sessions
`branch-verify.sh` resolves git root from session CWD, not the command's `cd` target. Using `cd <worktree> && git add` from a mya session (CWD on main) triggers a false denial. Use `git -C <worktree-path> add <files>` instead — the hook parses `-C` correctly.
Source: mya session 2026-04-14 / errata #145

### 2026-04-14: thebrana deploys by merging to main — no deploy command
`brana deploy` does not exist. The file system IS the deployment: skills, hooks, rules, agents load from disk at session start. To deploy: `git merge <branch>` from thebrana main. Worktrees are the staging area.
Source: mya session 2026-04-14 / errata #146
