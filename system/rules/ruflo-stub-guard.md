...
| `mcp__ruflo__agentdb_controllers` and all `agentdb_*`, including `agentdb_semantic-route` | All fail: `AgentDB bridge not available`. Root cause confirmed 2026-08-12 (t-2757): installed `@claude-flow/memory` double-exports `ControllerRegistry`, crashing the bridge on import — a packaging bug, fixed upstream (not in our pinned version), not a "v3.5 vs v3.6" split. | Use `mcp__ruflo__memory_search` / `memory_store` instead |
