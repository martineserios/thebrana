...
# Scans ## headings for repeated slugs. Surface only — no auto-fix.
# ══════════════════════════════════════════════════════════════
pass_patterns_dedup() {
    log "Pass 5: patterns.md dedup"
    if [[ ! -f "$PATTERNS_FILE" ]]; then
        log "  patterns.md not found ($PATTERNS_FILE), skipping"
        printf '0\n'
        return
    fi

    local slugs dupes count=0 findings=""
    slugs=$(grep '^## ' "$PATTERNS_FILE" | sed 's/^## //')

    dupes=$(echo "$slugs" | sort | uniq -d)

    while IFS= read -r slug; do
        [[ -z "$slug" ]] && continue
        log "  dedup: duplicate slug '$slug' in patterns.md"
        findings="${findings}- **${slug}** — duplicate slug in patterns.md"$'\n'
        count=$((count + 1))
    done <<< "$dupes"

    log "Pass 5 done: $count duplicate pattern slugs"
    printf '%d\n%s' "$count" "$findings"
}

# ══════════════════════════════════════════════════════════════
# PASS 6: knowledge-staging.md cap check
# Reads warn-at from frontmatter comment; warns when entry count >= threshold.
# Surface only — no auto-fix.
# ══════════════════════════════════════════════════════════════
pass_staging_cap() {
    log "Pass 6: knowledge-staging.md cap check"
    if [[ ! -f "$STAGING_FILE" ]]; then
        log "  knowledge-staging.md not found ($STAGING_FILE), skipping"
        printf '0\n'
        return
    fi
    ...
    if [[ "$count" -ge "$cap" ]]; then
        ...
        msg="knowledge-staging.md has $count entries (warn-at: $warn_at, cap: $cap) — promote or prune"
        log "  WARNING: $msg"
    else
        log "  knowledge-staging.md: $count/$cap entries (warn-at: $warn_at) — OK"
    fi

    log "Pass 6 done: $count entries"
    printf '%d\n%s' "$count" "$msg"
}
...
write_report() {
    ...
    local patterns_dedup_output="${5:-}"
    local staging_cap_output="${6:-}"

    local contra_count contra_findings surface_count surface_findings
    local patterns_dedup_count patterns_dedup_findings staging_cap_count staging_cap_findings
    ...
    patterns_dedup_count=$(echo "$patterns_dedup_output" | head -1)
    patterns_dedup_findings=$(echo "$patterns_dedup_output" | tail -n +2)
    staging_cap_count=$(echo "$staging_cap_output" | head -1)
    staging_cap_findings=$(echo "$staging_cap_output" | tail -n +2)
...
| patterns.md duplicate slugs (Pass 5) | $patterns_dedup_count |
| knowledge-staging.md cap (Pass 6) | $staging_cap_count entries |
...
## patterns.md Duplicate Slugs (Pass 5)

> Duplicate ## headings in patterns.md. Remove or rename the redundant entry.

${patterns_dedup_findings:-_No duplicates found._}

## knowledge-staging.md Cap Status (Pass 6)

${staging_cap_findings:-_Within limits._}
...
main() {
    ...
    local dedup_count contra_output imputed_count surface_output patterns_dedup_output staging_cap_output
    dedup_count=$(pass_dedup)
    contra_output=$(pass_contradiction)
    imputed_count=$(pass_imputation)
    surface_output=$(pass_surfacing)
    patterns_dedup_output=$(pass_patterns_dedup)
    staging_cap_output=$(pass_staging_cap)

    write_report "$dedup_count" "$contra_output" "$imputed_count" "$surface_output" \
        "$patterns_dedup_output" "$staging_cap_output"
    update_state
