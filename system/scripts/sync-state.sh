...
PRIVATE_REPO="${BRANA_PRIVATE_REPO:-$HOME/enter_thebrana/brana-knowledge}"
PRIVATE_STATE_DIR="$PRIVATE_REPO/backup/state"
PRIVATE_CACHE_PATHS=(
...
cmd_push() {
    ...
    # Private files -> the private repo only (t-3352). Deliberately NOT counted in
    # `changed` (that gates the public auto-commit) and never falls back to STATE_DIR.
    if [ -d "$PRIVATE_REPO/.git" ]; then
        for src in "${PRIVATE_CACHE_PATHS[@]}"; do
            sync_file "$src" "$PRIVATE_STATE_DIR/$(basename "$src")" || true
        done
    else
        log "skip private state — no private repo at $PRIVATE_REPO (not falling back to the public repo)"
    fi
...
cmd_pull() {
    ...
    # Private files (t-3352): restore the cache from the private repo. portfolio.md is
    # restored by brana-knowledge/restore.sh (its backup already holds ~/.claude/memory).
    if [ -d "$PRIVATE_REPO/.git" ]; then
        for dst in "${PRIVATE_CACHE_PATHS[@]}"; do
            if [ -f "$PRIVATE_STATE_DIR/$(basename "$dst")" ]; then
                mkdir -p "$(dirname "$dst")"
                sync_file "$PRIVATE_STATE_DIR/$(basename "$dst")" "$dst" || true
            fi
        done
    else
        log "skip private state — no private repo at $PRIVATE_REPO"
    fi

    # Companion files per project (registry comes from the restored cache, never the public repo)
    local portfolio="$HOME/.claude/tasks-portfolio.json"
    if [ -f "$portfolio" ]; then
        local paths
        paths=$(jq -r '
...
auto_commit_state() {
    ...
    # Only auto-commit in thebrana repo — and only the files this script itself maps as
    # public state (REPO_PATHS). This used to be `git add system/state/`, i.e. "publish
    # whatever lands in this directory", which put client fees and names into a PUBLIC
    # repo for six months (t-3352). An allowlist removes that capability instead of
    # denylisting today's two bad file names; a new or renamed file can no longer
