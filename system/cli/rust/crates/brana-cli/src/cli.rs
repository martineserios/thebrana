...
pub enum Commands {
    ...
    /// How to deploy brana (hint — ship dev->main, then ./bootstrap.sh from main; ADR-060)
    Deploy,
...
pub enum KnowledgeCmd {
    ...
    /// ruflo memory_entries DBs, then re-score the link-capture and
    /// intelligence-feed rows for project relevance (t-2620, t-3311).
    /// Idempotent: newest row per key wins; scoring is a full recompute.
    VectorSync {
        ...
        #[arg(long)]
        dest: Option<PathBuf>,
        /// Per-run cap on ruflo-side `project:<slug>` tag writes (default 25).
        /// Scoring itself is never capped — only the subprocess tag writes are.
        #[arg(long)]
...
pub enum WaveCmd {
    ...
    Pull {
        ...
        #[arg(long)]
        dry_run: bool,
        /// Fan-out width (ADR-090 §1 `N`). With --dry-run, report the
        /// WHOLE beat's decisions — every task an N-wide beat would claim,
        /// plus why it stops short of N. Without --dry-run and n > 1, run
        /// the real beat (t-3271): N sequential atomic pulls, each with its
        /// own lock and lease, reporting `pulled_task_ids` and why it
        /// stopped. n = 1 keeps the single-pull output unchanged.
