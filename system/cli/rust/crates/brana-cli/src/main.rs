...
fn main() {
    ...
    match args.command {
        ...
        Commands::Backlog { cmd } => match cmd {
            BacklogCmd::Next { tag, kind, limit, priority, task_type, effort, parent, epic, json } => run_or_exit(commands::backlog::cmd_next(&theme, tag, ve_str(&kind), limit, ve_str(&priority), ve_str(&task_type), ve_str(&effort), parent, epic, json)),
            BacklogCmd::Query {
                tag, status, kind, priority, effort, search, count, mut output, json,
                task_type, parent, branch, work_type, epic, ac_state, sort, role,
            } => {
                if json { output = "json".to_string(); }
                run_or_exit(commands::backlog::cmd_query(tag, ve_str(&status), ve_str(&kind), ve_str(&priority), ve_str(&effort), search, count, output, &theme, task_type, parent, branch, ve_str(&work_type), epic, ve_str(&ac_state), ve_str(&sort), role))
            },
            ...
            BacklogCmd::Wave { cmd } => match cmd {
                ...
                WaveCmd::Board { json, file } => run_or_exit(commands::backlog::cmd_wave_board(&theme, json, file)),
                WaveCmd::Set { wave_id, field, value, file } => run_or_exit(commands::backlog::cmd_wave_set(&wave_id, &field, &value, file)),
                WaveCmd::Ship { wave_id, file } => run_or_exit(commands::backlog::cmd_wave_ship(&wave_id, file)),
                WaveCmd::Pull { wave_id, dry_run, n, claimant, file } => run_or_exit(commands::backlog::cmd_wave_pull(&wave_id, dry_run, n, claimant, file)),
