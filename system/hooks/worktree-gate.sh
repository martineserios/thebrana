...
if [ -n "$DIRTY" ]; then
    ...
    if [ "$NON_BEHAVIORAL_ONLY" = true ] && [ -n "$ALL_DIRTY_FILES" ]; then
        ...
        deny "Worktree required: $DIRTY detected. Use \`git worktree add ../<repo>-${BRANCH_NAME} -b ${BRANCH_NAME}\` or \`claude --worktree ${BRANCH_NAME}\` instead of \`${USED_CMD}\`. To stash first (including untracked files): \`git stash push -u\`. See git-discipline rule."
    fi
