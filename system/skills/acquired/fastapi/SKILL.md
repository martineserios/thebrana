...
status: experimental
source: "https://skills.sh/fastapi/fastapi/fastapi"
acquired: "2026-05-17"
promoted: "2026-05-17"
---
# FastAPI
...
See the dependency injection reference (upstream `references/dependencies.md`, not installed locally) for detailed patterns including `yield` with `scope`, and class dependencies.
...
When needing to mix blocking and async code, see Asyncer in the other tools reference (upstream `references/other-tools.md`, not installed locally).

## Streaming (JSON Lines, SSE, bytes)

See the streaming reference (upstream `references/streaming.md`, not installed locally) for JSON Lines, Server-Sent Events (`EventSourceResponse`, `ServerSentEvent`), and byte streaming (`StreamingResponse`) patterns.

## Tooling

See the other tools reference (upstream `references/other-tools.md`, not installed locally) for details on uv, Ruff, ty for package management, linting, type checking, formatting, etc.

## Other Libraries

See the other tools reference (upstream `references/other-tools.md`, not installed locally) for details on other libraries:
