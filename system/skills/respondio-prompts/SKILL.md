...
> **Platform reference:** `brana-knowledge/dimensions/50-respond-io-platform.md` — authoritative source for limits, triggers/steps, variable namespaces, AI Agent actions (incl. Make HTTP Requests + Handle Calls), known limitations + workarounds, plan tier gates, recent changes.
> **Hard rules from that doc that must be enforced in every prompt:**
> - 4-part instruction framework (CONTEXT / ROLE & COMMUNICATION STYLE / FLOW / BOUNDARIES) — official Respond.io structure.
> - Actions must appear in BOTH Instructions AND Actions sections for best accuracy.
> - Chain **≤3 actions per agent turn**; more causes unpredictability.
> - Exact name matching for stages/fields/teams/closing-notes — no fuzzy match, silent failures on typos.
> - Canonical references: `@TeamName`, `$contact.fieldName`, `$agent.varName` (agent-scoped).
> - KB retrieval is keyword-only — front-load searchable terms in content, never rely on titles.
> - 10k char ceiling on instructions; 1k on action prompts; reserve 1.5–2k buffer.
> - Voice agents (Handle Calls) can't be handed off mid-call and don't receive HTTP response payloads.
...
9. **Single source of truth**: no duplicated prices, staff names, addresses across agents
...
| Explicit conditions | "When pain reported, assign to @[Exact User Name]" | "Escalate if serious" |
...
| 7 | Workflow | `Trigger workflow ![name]` | No variable passing. Must be published. Set fields first. **Must be configured in its own "Trigger Workflows" Action block (separate toggle + separate "When and how" condition field in the live AI Agent settings) — merely describing it in the main Instructions/FLOW text does nothing.** Confirmed root cause of a real production bug (somos_mirada, 2026-07-18, t-123): an agent's FLOW text said `→ Trigger Workflow "W2"`, the Trigger Workflows toggle was ON, but its dedicated condition field was empty — the trigger never fired, for the entire time it was "documented" as working. Always give this action its own `action-N-trigger-workflow.md` file, same as every other action type, instead of folding it into the flow prose. |
...
- Shared data in a single file attached to all agents (prices, staff, scope)
...
| Last follow-up | Must chain to closing action (e.g., route to the closing agent) |

Design rules:
- UI configures follow-up messages and timing
- Instructions handle routing when follow-ups exhaust ("after follow-up with no response → route to the closing agent")
...
- [ ] Every `!workflow_name` reference has its own `action-N-trigger-workflow.md` file — never only prose in the FLOW section. In the live UI, confirm the "Trigger Workflows" toggle is ON *and* its condition field is non-empty, not just the toggle.
...
- [ ] No circular routing (e.g., the closing agent never routes back to the entry-point classifier)
...
9. **When in doubt, escalate to human** — better to over-escalate than hallucinate high-stakes info (medical, pricing, legal).
