...
# --- fixture: git repo with origin, commits touching system/cli/rust ---
make_repo() {
  local root="$1"
  ...
  (cd "$root/repo" \
    ...
    && mkdir -p system/cli/rust/src docs \
...
write_manifest "$ROOT/manifest.json" "$COMMIT" "aaaa"
OUT="$(STUB_MANIFEST="$ROOT/manifest.json" STUB_BIN_SHA=bbbb run_check "$ROOT")"; RC=$?
...
# Test 4: main moved under system/cli/rust since manifest commit -> DRIFT, exit 1
ROOT="$(mktemp -d)"; make_repo "$ROOT"; make_ssh_stub "$ROOT"
OLD="$(git -C "$ROOT/repo" rev-parse HEAD)"
(cd "$ROOT/repo" && echo 'fn fix(){}' >> system/cli/rust/src/main.rs \
  && git add -A && git commit -q -m "fix(cli): newer" && git push -q origin HEAD 2>/dev/null)
write_manifest "$ROOT/manifest.json" "$OLD" "cccc"
OUT="$(STUB_MANIFEST="$ROOT/manifest.json" STUB_BIN_SHA=cccc run_check "$ROOT")"; RC=$?
...
ROOT="$(mktemp -d)"; make_repo "$ROOT"; make_ssh_stub "$ROOT"
COMMIT="$(git -C "$ROOT/repo" rev-parse HEAD)"
write_manifest "$ROOT/manifest.json" "$COMMIT" "dddd"
OUT="$(STUB_MANIFEST="$ROOT/manifest.json" STUB_BIN_SHA=dddd run_check "$ROOT")"; RC=$?
...
write_manifest "$ROOT/manifest.json" "$OLD" "eeee"
OUT="$(STUB_MANIFEST="$ROOT/manifest.json" STUB_BIN_SHA=eeee run_check "$ROOT")"; RC=$?
if [[ "$RC" -eq 0 ]] && grep -qi "ok" <<<"$OUT"; then
